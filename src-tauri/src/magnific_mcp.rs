//! 마그니픽 **MCP** 로 직접 뽑기.
//!
//! API 로 한 번에 뽑을 때 마그니픽은 MCP 로 부릅니다 — 캐릭터 그림을 뽑고, 고르고,
//! 구도를 잡고, 레퍼런스와 합쳐 씬 영상까지 한 줄로 이어야 하기 때문입니다. «무제한» 을
//! 못 쓰고 건당 값을 치르는 것은 자동으로 뽑는 대가입니다.
//!
//! # 여태 길과 무엇이 다른가
//!
//! 여태는 마그니픽 **데스크톱 창**에 CDP 로 붙어 생성기를 «차려 놓기» 만 했습니다
//! (`magnific_cdp`). 뽑는 것은 사람이 그 창에서 눌렀고, 결과는 동기화 폴더로 돌아왔습니다.
//! «무제한» 이 웹앱 안에서만 적용되기 때문에 고른 길입니다.
//!
//! 이 모듈은 **끝까지 뽑습니다.** 사람 손이 한 번도 안 듭니다. 대신 건당 과금입니다 —
//! 밤새 통째로 돌리려면 치러야 하는 값입니다. 두 길은 **함께 남습니다**: 손으로 골라 가며
...
/// 기본 브라우저로 주소를 엽니다.
///
/// 웹뷰의 `window.open` 은 Tauri 안에서 막혀 있습니다 — 눌러도 창이 뜨지 않습니다.
...
pub async fn magnific_upload(path: String) -> Res<String> {
    ...
    */
    ...
    /*
      주소 칸의 이름이 `proxyUploadUrl` 입니다(실제 답을 받아 확인).
...
한 번에 뽑기가 자꾸 빈손으로 끝났습니다 — 그림은 마그니픽에 멀쩡히 만들어져
