//! **ComfyUI 업스케일 다리** — `lib.rs` 에서 떼어 냈습니다.
//!
//! 앱이 품은 로컬 엔진(`upscale.rs`)과는 별개입니다. 이쪽은 «쓰는 사람이 이미 돌리고 있는
...
// ─────────────────────────────────────────────────────────────────────────────
//
// SeedVR 같은 공짜 업스케일 모델은 ComfyUI 에서만 도는데, 그 힘을 우리 앱에서도 쓰려고
// 놓은 다리입니다(4K 까지 키울 수 있습니다).
//
// 모델을 앱에 품지 않습니다. 이미 ComfyUI 를 돌리고 있는 사람이라면 거기서 만든 워크플로
...
pub async fn comfy_upscale_image(
    ...
    /*
      **결과는 원본과 같은 폴더에만 씁니다.**
      ...
      점검에서 드러났습니다 — 여기에는 폴더 범위 검사가 **하나도 없었습니다.**
