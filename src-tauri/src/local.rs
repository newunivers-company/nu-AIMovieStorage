...
//!
//! 컴피UI 를 거치지 않고 업스케일 엔진과 똑같이 **앱이 직접** 돌립니다 — 바깥 프로그램을
//! 하나 더 깔고 켜 두게 하면, 거기서 어긋난 것은 우리가 손댈 수 없습니다.
//!
//! # 여기에는 살림이 없습니다
//!
...
///
/// 캐릭터마다 영상을 따로 짝짓는 길과, 한 영상에서 인물을 골라 짝짓는 길이 **섞여** 쓰입니다.
///
...
///
/// 곡에 맞춰 컷을 늘어놓는 쓰임입니다. 영상 고르기와 같은 길 — 브라우저 파일 창은 경로를 주지 않아서
...
/* ─────────────────────── 이 컴퓨터가 무엇을 돌릴 수 있나 ───────────────────────
  설정에서 디스크와 GPU 를 재어 **이 기계가 감당할 모델**만 권합니다. 같은 엔진이라도
  VRAM 이 작은 기계에서는 양자화 판으로 내려야 해서, 고를 때 이미 갈려 있어야 합니다.
