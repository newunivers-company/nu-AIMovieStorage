...
//! # 이 파일을 다시 쓴 경위
//!
//! 원본은 D 드라이브 사고로 잃었습니다. 복구본은 파일 이름과 크기만 남고
//! 본문이 0으로 채워져 있었습니다. 대신 대화 기록에 프런트가 이 명령들을
//! 어떻게 부르는지가 전부 남아 있어서, **호출부에서 역산해** 다시 썼습니다.
//! 명령 이름·인자 이름·반환형은 기록에서 확인한 그대로입니다.
//!
//! 그래서 이름과 모양은 맞지만, **폴더 구조 같은 세부는 짐작한 곳이 있습니다.**
//! 프런트를 되살리면서 어긋나는 곳이 나오면 여기를 고치면 됩니다.
//!
...
/// 쓰는 모든 명령이 줄줄이 죽어, 쓰는 쪽에서는 업스케일 한 번 실패한 뒤로 앱이
/// 아무것도 안 받는 것처럼 보입니다. 앱을 닫을 때도 같은 자리에서 터집니다.
///
/// 여기서 지키는 값들은 **목록·손잡이 모음**이라, 중간에 패닉이 나도 «반쯤 고쳐진»
/// 상태가 될 뿐 못 읽을 값이 되지는 않습니다. 그래서 독을 무시하고 이어 갑니다.
///
/// `upscale.rs` 에 이 처리가 **다섯 군데만** 있고 서른한 군데는 그냥 `unwrap()`
/// 이었습니다 — 여덟 곳은 맞고 한 곳만 틀린 모양이라 한 벌로 모읍니다(규칙 1).
...
pub(crate) fn safe_name(name: &str) -> String {
    ...
    /*
      ...
      인물 하나를 지우려다 **모든 인물 폴더** 가 지워집니다(실제로 재현했습니다).
      이 저장소가 생긴 사고와 같은 모양이라 이중으로 막습니다.
    */
    if cleaned.is_empty() {
        return "이름없음".to_string();
    }
...
///
/// 한 장을 여섯으로 나누면 원본까지 여덟 장이 한 목록에 섞여, 무엇이 낱장이고 무엇이
/// 원본인지 가릴 길이 없어집니다. `ref/` 처럼 주인 폴더 **안** 에 두어 이름 바꾸기·지우기가
/// 주인 폴더 단위로 그대로 따라오게 합니다.
...
/// 폴더를 갈라 두면 «이건 돔에 두르는 그림» 을 자리로 가립니다.
...
fn asset_layout(category: &str) -> (&'static str, Option<&'static str>, bool) {
    match category {
        ...
        // 공용 에셋은 **캐릭터 폴더 안** «공용에셋» 에 둡니다 — 개별 인물 폴더가
        // 아니라 그 옆자리라, 인물을 지워도 여럿이 나눠 쓰던 에셋이 같이 사라지지
        // 않습니다.
        ...
        */
        ...
        /*
          BGM 은 **영상 프로젝트와 따로** 삽니다 — 곡 하나를 여러 작품이 나눠 쓰기 때문입니다.
        ...
        */
        ...
        /*
          모션 캡처에 올린 **영상**과 그 **분석 결과**. 둘 다 프로젝트 폴더 안에 둡니다 —
          작품별로 모여 있어야 나중에 무엇을 분석해 썼는지 되짚을 수 있고, 프로젝트를
          통째로 옮길 때 같이 갑니다.
        */
        ...
        /*
          **시나리오·기획안 원본** — 프로젝트 폴더 안 `DOCU/` 에 둡니다.
...
fn import_project_asset(request: ImportAssetRequest) -> Res<String> {
    ...
    /*
      ...
      원본은 대개 내려받기 폴더에 있고, 옮겨 버리면 다른 프로젝트에서 다시 쓸 수 없습니다.
    */
...
/*
  ...
  프로젝트 폴더 안인지 ensure_inside 로 확인하는 명령만 둡니다.
*/
...
fn migrate_project_layout(base_directory: String, project_name: String) -> Res<Vec<(String, String)>> {
    ...
    // 한때 `<프로젝트>/asset/<에셋>/` 에 두었던 것을 지금 자리인
    // `<프로젝트>/character/공용에셋/<에셋>/` 로 옮깁니다 — 옛 저장본을 열었을 때
    // 에셋이 통째로 안 보이면 안 됩니다. 폴더째 옮기고 안의 파일 짝을 돌려줍니다.
...
// ─────────────────────────────────────────────────────────────────────────────
//
// 마그니픽 데스크톱은 웹앱을 Tauri 로 감싼 것이라 딥링크·CLI 같은
// 공식 통로가 없습니다. 그래서 두 가지로 잇습니다.
//
// 1. 보내기 — 프롬프트를 클립보드에 넣고 마그니픽 창을 앞으로 가져와 Ctrl+V.
// 캔버스(Spaces)를 눌러 두었으면 텍스트 노드가 됩니다(실제로 확인한 동작).
...
/// «지운 파일은 다시 내려받는다» 고 스스로 밝히고 있습니다(설정 화면 안내문).
...
/// 내려줍니다 — 보낸 그림이 후보함에 그대로 되돌아옵니다. 보낼 때
