 ...
 *
 * 로컬 모델은 ComfyUI 를 끼지 않고 앱이 직접 돌립니다 — 업스케일 엔진과 같은 살림입니다.
 * 바깥 프로그램을 따로 깔고 켜 두어야 하면 그 순간 쓰는 사람이 반으로 줍니다.
 *
...
export default function LocalEnginesPanel({
  ...
  /*
    이 컴퓨터를 **한 번** 읽습니다 — 빈 자리와 GPU 를 봐야 받을 수 있는 모델만 권할 수
    있기 때문입니다. 카드마다 읽으면 엔진 수만큼 nvidia-smi 가 돕니다.
  */
 ...
 * 이 컴퓨터가 무엇인지 한 줄로.
 *
 * 만드는 쪽 기계는 넉넉해도 받아 쓰는 기계는 VRAM 에서 막힙니다.
...
function HardwareRow({
  ...
  return (
    ...
    >
      ...
      {/*
        ── 정밀도 ────────────────────────────────────────────────────
        기본이 «자동» 이고, 워커가 올릴 때 이 GPU 를 보고 정합니다 — 쓰는 사람이
        정밀도를 알아야 할 까닭이 없습니다.
...
function LocalEngineCard({
  ...
  /*
    «가중치 미리 받기» — 설치 때 받는 길과 **둘 다** 둡니다.
    이 개편 전에 깐 엔진은 설치 때 받는 길을 이미 지나쳤기 때문입니다. 미니맥스 H3 의
 ...
 *
 * 모션 캡처 모델을 넣으며 생겼습니다. 토큰을 넘기는 길이 없어서 게이트 저장소 엔진은 설치해도 첫 실행에서 401 로
 * 멈췄습니다. API 키와 같은 자리(설정 폴더 파일)에 두고, Rust 가 워커를 띄울 때 환경 변수로만 넘깁니다(`upscale.rs` ensure_worker).
 * 화면에는 끝 네 글자만 보입니다.
 */
function HuggingFaceTokenRow() {
  // 줄의 모양은 `ApiTokenRow` 한 벌(Civitai 키와 같이 씀). 여기서는 무슨 토큰인지와, 저장 뒤 워커를 내리는 일만.
  return (
    <ApiTokenRow
