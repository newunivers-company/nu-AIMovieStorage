...
export default function BlueprintTogglePanel({
  ...
  /*
    ...
    함께 뜨면 고를 일이 없는 것을 계속 지나쳐야 합니다(). 목록만 좁히고 **저장값은 건드리지 않습니다** — 씬 탭에서 같은 카드를 열면 그대로입니다.
  */
  const groups = only
    ? all
        .map((group) => ({
  ...
  return (
    <section className="space-y-2" data-tour="card-blueprint">
      ...
      {/*
        칸이 **하나뿐이면 폭을 다 씁니다.** 구도잡기에서 전개도만 뽑을 때는 그룹이 하나라, 두 칸 격자에 두면 오른쪽이 텅 빕니다.
      */}
      <div
        className={cn(
          "grid gap-2",
