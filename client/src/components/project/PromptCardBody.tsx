...
>({
   ...
   *
   * 원래는 카드 맨 아래(생성 이미지 위)에 있었는데 분석 위로 올렸습니다 —
   * 이름·외형 다음에 성격, 그다음 분석·프롬프트 순서가 사람이 인물을 정해 나가는
   * 순서와 같습니다.
   */
  beforeAnalysis?: React.ReactNode;
  /**
   * 가져올 수 있는 다른 분석 — 변형 창에서 부모와 형제 변형의 분석을 줍니다.
   * 같은 부모의 변형은 생김새가 같아 분석을 나눠 쓸 때가 많습니다. 다만 옷이나 나이가
   * 달라지는 변형도 있어 **늘** 같지는 않으므로, 자동으로 채우지 않고 단추로 고릅니다.
   */
   ...
   * 들어가 ref/ 폴더에 `ref_` 이름으로 저장됐는데, 그러면 생성 이미지 목록에 안 보여
   * 다른 변형에서 집을 수가 없었습니다.
   */
  cropSave?: {
    ownerName: string;
   ...
   *
   * - 전개도를 만들 때 «첫 레퍼런스 프롬프트» 는 뺍니다 — 구도잡기에서 연 카드는 늘 처음입니다.
   * - 구성 칸은 등장방형(실내·실외, 안쪽면·바깥쪽면)만 남깁니다. 여기서 뽑은 그림은 바로
   * 잘라 방에 붙이는 것이라 나머지 구도 선택지가 쓰일 자리가 없습니다.
   * - 벽 그림은 **벽 크기와 인물까지의 거리**로 뽑는 것이라 구성 칸 자체가 필요 없습니다.
   *
  ...
  /*
    레퍼런스 타일에 있던 «업스케일 ▾» 은 없앴습니다. 자르면 그림이 작아지니 키우기는 자르기와
    한자리에 있는 편이 낫습니다 — 키우기는 타일의 가위로
    ...
    <div className="space-y-3">
      ...
      */}
      ...
      {/*
        ...
        **이미지 분석 바로 위**입니다. 아래는 전부 «그림이 있을 때» 의 일이고
        ...
        그 앞에 섭니다. 적어 둔 특징만 가지고 레퍼런스 없이 첫 프롬프트를 지어 주는
        칸이라, 그림 한 장도 없는 맨 처음에 쓰는 것입니다.
      ...
      */}
      ...
      {/*
        ...
        (프롬프트와 같음). 요청이 오류로 끝나 칸이 비어도 여기서 되살립니다.
      */}
      <PromptHistoryShelf
        title="받아 둔 분석"
      ...
      {/*
        벽 그림은 **벽 크기와 인물까지의 거리**로 뽑는 것이라 구성 칸이 필요 없습니다 — 구도는 이미
        3D 쪽에서 정해졌고, 여기서 또 고르면 그 값과 싸웁니다. 방·돔은 전개도/파노라마 칩만 남깁니다.
      */}
      {placeScope !== "wall" && (
      <BlueprintTogglePanel
        ...
        spaceKind={spaceKind}
        /*
          등장방형(전개도) 칩을 켜도 **이미지 모델은 안 바꿉니다.** 한때 GPT 로 바꿔 줬는데, 그 모델은 요금이 들어
          켜는 줄도 모르고 값을 치르게 됩니다. 문서 16 §10 기록으로도 기본 모델이 실외 50 m 정육면체(정사각 칸)는 성공했고,
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {/*
          지금 칸에 적힌 프롬프트 그대로 이 컴퓨터의 모델로 한 장 뽑습니다 — 밖으로
          옮겨 붙이지 않아도 결이 맞는지 바로 봅니다. 캐릭터·배경·에셋·변형이 같이 씁니다 —
      ...
      {/*
        ...
        예전에는 레퍼런스로 바로 들어가 ref/ 폴더에 `ref_` 로 저장됐는데, 거기 들어가면
        «이 캐릭터의 생성 이미지» 줄에 안 떠서 다른 변형에서 집을 수가 없었습니다.

        지운 판(`"erase"`)도 같이 들어갑니다. 잘라낸 칸을 생성 이미지로 옮기면서
        `"crop" | "panorama"` 만 통과시키기 시작해 지운 판(그때는 `"mark"`)이 버려졌습니다.
        ...
        로 봅니다 — 실제로 지우기를 한 그림이 깨진 아이콘으로 떴습니다.
          ...
          onSaved={(files) => {
            ...
            */
            ...
            /*
              ...
              레퍼런스로 **또** 붙입니다 — 한 그림이 두 목록에 앉는 «두 개로 불어나던 것» 이
 ...
 *
 * 미드저니를 뺀 나머지는 전부 마그니픽 안에서 돌아갑니다. 그래서 «구성» 은
 * **고른 모델을 그대로** 마그니픽 생성기에 얹습니다(`magnificModelOf`).
    ...
    >
      ...
      {open && (
        <div className="space-y-2 px-3 pb-3">
          ...
          {/*
            한글·영문을 **따로** 둡니다. 모델마다 잘 듣는 말이 달라서 — 나노 바나나·GPT 는
