...
export default function CutVideoShelf({
   ...
   *
   * 영상이 들어오는 길이 «마그니픽 후보함에서 채택» 과 «로컬 생성» 둘뿐이라, 클링·Veo 처럼
   * 다른 데서 뽑아 온 mp4 는 넣을 자리가 아예 없었습니다. 이 앱은 밖에서 뽑아 다시 등록하는
   * 흐름이 기본이라, 그림에 있는 등록 길이 영상에도 있어야 합니다.
   */
  canAdd?: boolean;
      ...
      for (const file of movies) {
        /*
          **원본 파일 이름은 넘기지 않습니다.**
          ...
          이름을 붙이는 공통 함수는 멀쩡한데, `stem` 에 원본 이름(`KakaoTalk_20260911_…`)을 넘기면
          그 이름이 그대로 굳어 규칙 5 의 «주인_번호» 를 비켜 갑니다. `stem` 을 비워야 주인 이름이
          앞에 붙고 Rust 가 `_001` 을 올립니다 — «수화의 숲_001».
  ...
  return (
    ...
    >
      ...
      {/*
        ── 큰 화면 ──────────────────────────────────────────────────
        선반의 칸은 200px 이라 얼굴도 자막도 안 보여, 뽑은 영상을 고를지 말지 정할 수가 없습니다.
        고른 것을 화면 가득 띄웁니다.
      */}
      {big && (
        <div
