 ...
 *
 * 예전에는 손으로 했습니다(가위 → 파노라마 탭 → «여섯 면 만들기»). 규격이 정해진 그림이라
 * 매번 같은 손놀림이었으므로, 전개도와 같은 자리에서 자동으로 합니다.
 */
...
export function autoEquirectOf(
   ...
   *
   * 50 m 로 프롬프트를 뽑아 그림을 받은 뒤 칸의 한 변을 100 으로 바꿔 두면, 세트가 100 m 방으로 서서 나무가
   * 사람의 두 배로 커 보입니다. 그림이 담은 공간은 생성기가 읽은 프롬프트의 크기이지 지금 칸의 값이 아닙니다.
   */
...
/**
 * 자동 커팅을 켤 것인가. 기본은 **켜짐** — 매번 같은 손놀림을 대신하는 것이 이 기능입니다.
 *
 * 끄는 길을 두는 까닭: 판정이 아무리 조여도 «전개도로 보이는데 아닌 그림» 이 언젠가
 * 나옵니다. 그때 설정에서 끄고 손으로 자를 수 있어야 합니다.
 */
 ...
 *
 * 배경 6면 그림은 우리가 규격을 정해 뽑으므로, 자르는 자리도 늘 같습니다.
 *
 ...
 * 세트 카드를 남깁니다.
 *
 * 실측으로 실내 안쪽·바깥·실외 전개도 셋은 전부 잡고, 전개도 틀은 «단색» 에서
 * 걸러 내는 것을 확인했습니다.
 *
 * # 전개도 원본은 그대로 둡니다
 *
 * 전개도와 6면 그림은 **둘 다** 남습니다. 자른 뒤에도 전개도가 목록에 있습니다 — 다시
