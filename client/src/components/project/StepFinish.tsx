 ...
 * # 여기서는 아무것도 만들지 않습니다
 *
 * 이 자리가 하는 일은 셋뿐입니다 — 씬 구성 탭에서 구운 스토리보드를 씬마다 보여 주고,
 * 시트마다 인쇄 단추를 열어 주고, 그 스토리보드로 뽑은 최종 영상을 여기에 등록받습니다.
 *
 * 그래서 걷어낸 것:
 ...
 * - **«작업실로»** — 작업실 페이지 자체가 앞 단계와 내용이 겹쳐 통째로 걷어냈습니다.
 * 그 자리는 단계 줄 아래의 «프로젝트 목록으로» 하나뿐입니다 — 본문에도 같은 단추를
 * 두었더니 한 화면에 같은 단추가 둘이라, 긴 쪽을 지웠습니다.
 *
  ...
  const scenes = useMemo(
    () =>
      draft.scenes.map((scene, index) => {
        ...
        /*
          씬 영상만이 아니라 **컷 영상도** 모읍니다. 스토리보드로 만든 씬 영상이 아니라
          컷 하나를 영상으로 뽑는 길도 있는데, 컷 영상은 컷 카드 안에만 있어 씬별·컷별로
          전체를 훑어볼 자리가 없었습니다.
        */
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={printPath ? "space-y-4 print:hidden" : "space-y-4"}>
        ...
        {/*
          ── 한 번에 뽑기 ─────────────────────────────────────────────
          마그니픽으로 최종 씬 영상까지 한 번에 뽑습니다 — 그림과 영상은 따로 골라 뽑습니다.
        ...
        {scenes.length === 0 ? (
          ...
          scenes.map((scene) => (
            ...
            >
              ...
              {/*
                ── 이 장면에서 나온 영상 ────────────────────────────────
                밖에서 뽑아 온 것도 **여기서 바로 등록**합니다 — 올리거나 불러올 자리가
                없으면 확인 탭에 최종 영상이 영영 모이지 않습니다.
                등록한 것은 그 장면의 «씬 영상» 으로 붙습니다 —
              ...
              */}
              ...
              {/*
                ...
                둘을 합쳐 다시 늘어놓으면 같은 영상이 한 화면에 두 번 뜹니다.
              */}
