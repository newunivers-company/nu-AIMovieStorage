...
export interface LineageOwner {
   ...
   * (검토 2026-09-08). 파일이 아직 없을 때는 접두 충돌 검사(`renameOwnedAssetFiles` 의 foreignPaths)가
 ...
 *
 * 셋의 접두 규칙이 같아서 이름이 같으면 폴더 안에서 파일 이름 공간이 하나가 됩니다(검토 2026-09-08).
 ...
 * (2026-09-05) 를 세 번 고쳤습니다. 한 군데를 빠뜨리면 그 갈래만 예전 동작으로
 * 남고, 아무도 모릅니다.
 *
...
export function useEntityLineage<T extends LineageEntity>({
  ...
  const folderFor = (entity: T): LineageFolder => {
    ...
    return {
      ...
      // 폴더 읽기가 주인 폴더 전체를 제 것으로 줍습니다(검토 2026-09-08). 임시 접두는 `냥이_에셋`.
  ...
  const remove = async (entity: T) => {
    /*
      **화면에서 지우면 폴더의 원본도 지웁니다.** (CLAUDE.md 규칙 3)
      ...
      되살아나고, 탐색기에는 쓰지 않는 인물 폴더가 쌓입니다. 사용자가 두 번
      말한 것입니다 — 「캐릭터 삭제 했는데 데이터는 그냥 남아 있네?」
      배경·공용 에셋도 같은 규칙입니다.
    */
    /*
      다른 원본(인물·장소인데 주인이 있음)은 갈래 문구를 그대로 쓰면 「이 인물의 … 시트가 모두 사라집니다」 가 되어
      주인을 통째로 지우는 줄 오해합니다(검토 2026-09-08). 실제로는 제 파일만 지우니 그렇게 말합니다.
  ...
  const removeVariation = async (entity: T, id: string) => {
    ...
    // 자식의 정체성 기준이라 남깁니다. (규칙 3, 2026-09-05 검증)
