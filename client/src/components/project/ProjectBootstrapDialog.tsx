 ...
 * 들어갑니다. 인물과 배경 설정을 하나하나 손으로 짜 넣기는 쉽지 않으니, 기본 뼈대를 한 번에
 * 받아 두고 보면서 고치는 쪽이 빠릅니다.
 *
 * # 왜 두 번 부르는가
 *
 ...
 * # 받은 답은 **저절로 들어갑니다**
 *
 * 창이 닫혀 있어도, 다른 프로젝트를 보고 있어도 들어가야 하므로 붓는 일은 살림이 합니다
...
export default function ProjectBootstrapDialog({
  ...
  /*
    ...
    가리킬 자리가 이 창에 가려 있으면 손이 닿지 않습니다. 규칙은 `useTutorialPanel` 한 곳에 있습니다.
  */
   ...
   *
   * 도는 중에 빠져나가 다른 작품을 만들 수 있어야 하고, 돌아오면 그 작품의 진행이 그대로
   * 있어야 합니다. 이 창은 «주제 설정» 단계 안에
  ...
  */
  ...
  const start = async (detailsOnly = false) => {
    /*
      ── 지우는 일은 **여기서** 확인받습니다 ────────────────────────────
      «적용» 단추를 따로 두지 않고 «만들기» 의 체크 하나로 끝냅니다.

      받은 뒤에 «적용» 을 기다리면, 답이 다 나왔는데 사람이 그 자리에 없어서 아무 일도
      안 일어납니다 — 그림도 영상도 안 뽑히는 것처럼 보이던 일이 이것이었습니다(«비우고 새로»
    ...
    */
    ...
    /*
      ...
      이 단추(«상세만 다시 만들기»)를 늘 보이게 바꾸면서 드러났습니다 —
    ...
    <div className={`${MODAL_BACKDROP} z-50`} style={MODAL_BACKDROP_STYLE}>
      <div className={modalCard("medium")} style={MODAL_CARD_STYLE}>
        ...
        >
          ...
          {/*
            **도는 중에도 닫을 수 있습니다.** 도는 동안 다른 작품도 만들 수 있어야 하니까요.
        ...
        <div className="composition-scroll min-h-0 flex-1 space-y-3 overflow-y-auto px-4 py-3">
          <label className="block space-y-1">
            ...
            {/*
              **이름표가 제대로 걸렸는지 보여 줍니다.**
              ...
              걸렸는지 아닌지는 색으로 말하는 것이 빠릅니다. 다만 글 안에서 색을
          ...
          {/*
            올린 그림·영상. 글만으로는 「이 춤」 을 말할 수 없습니다 — 댄스 커버가 그런
            보기입니다.
          */}
          {/*
...
${text}` : text })
          ...
          {/*
            ── 무엇으로 뽑을까 ──────────────────────────────────────────
            모델을 확인 단에서 고르게 두면 설정과 고르개가 갈라집니다.
          ...
          */}
            ...
            >
              ...
              */}
              ...
              {/*
                ...
                되므로(컷 하나가 그대로 영상 한 편이 되기도 합니다), 적용하기
          ...
          {/*
            ── 지난 답 ──────────────────────────────────────────────────
            API 로 받아 온 답이라 잃으면 다시 돈을 들여야 합니다 — 여기도 이력을 남깁니다.
          ...
          <div className="flex shrink-0 items-center gap-2">
            {/*
              **1단계를 건너뛰고 2·3단계만** 다시 부릅니다.
              ...
              그러면 앞 판이 **성공했을 때 단추가 사라져서**, 씬·컷만 다시 받고 싶어도
              1단계(가장 비싼 단)부터 통째로 다시 도는 수밖에 없었습니다.
            ...
            */}
            {/*
              ── 그림·영상만 다시 ────────────────────────────────────────
              «상세만 다시 만들기» 뿐이면 모델만 바꿔 그림을 다시 뽑으려 해도 API 를 또 씁니다.
            ...
            */}
            ...
            {/*
              ...
              상황·주변 환경·동작·표정 묘사가 얇으면 API 를 쓴 뜻이 없습니다.
