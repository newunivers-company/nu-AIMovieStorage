...
/*
  ...
  요청을 보내야 해서 훅·컴포넌트 밖으로 꺼냈습니다 — 두 벌로 두었더니 화면에서 고친 규칙이
  일괄 생성에는 안 닿았습니다. 여기서는 화면만 아는 것(캡처 저장·토스트·잠금)을 보태 부릅니다. 규칙 1.
*/
import {
  cutCharacterRefs,
  cutLinkInput,
...
export default function CutCard({
   ...
   *
   * 영상만 모델을 받고 그림은 안 받고 있었습니다 — 같은 컷인데 한쪽만 모델에 맞춰
   * 쓰이고, 그림 쪽은 아무 모델 문법으로나 나갔습니다.
   */
  imageModel?: string;
  /**
   * **영상** 화면비. 그림과 다를 수 있습니다(그림 16:9 · 영상 9:16).
   *
   * 점검에서 드러났습니다 — 영상 프롬프트에 그림 비율이 가고 있었습니다.
   */
  videoAspect?: string;
   ...
   *
   * 같은 공간에서 구도만 바꾸는 컷이 흔한데, 부를 때마다 인물을 다시 세우게 할 수는 없습니다.
   * 그래서 **구도 상태를 통째로** 복사합니다 — 방·소품·인물 자리까지. 불러온 뒤 배경만 갈아도 됩니다.
   */
  savedShots?: {
    id: string;
    label: string;
   ...
   *
   * 한 컷을 쓰는 데 드는 재료는 하나가 아닙니다 — 인물마다 **고른 시트**, 씬 설명에서 온
   * 배경과 상황, 구도잡기를 썼다면 저장한 구도, 거기에 대사·연기 지시·VFX 까지 다 들어가야
   * 그 컷이 무엇인지 말이 됩니다.
   *
   ...
   *
   * 방으로 구도를 잡고, 그 구도를 레퍼런스 삼아 인물을 얹어 그림으로 뽑습니다 —
   * 프롬프트까지 함께 올립니다.
   *
  ...
  /*
    ── 컷마다 켜고 끄는 두 스위치 ────────────────────────────────────────
    구도는 컷마다 넣고 뺄 수 있어야 하고, 넣고 뺀 대로 프롬프트와 @태그가 함께 달라져야 합니다.
  ...
  */
  ...
  const gatherCutRefs = async () => {
    ...
    */
    ...
    /*
      ...
      안 맞으니 배경을 제 나름대로 다시 해석합니다(실제로 그렇게 나온 적이 있습니다).
    ...
    */
    ...
    /*
      ...
      그림은…」 처럼 순서로 적으면 아무 그림도 안 걸립니다 — 그래서 프롬프트를 쓸 때부터
      `@` 규칙에 맞춰 적습니다.
    ...
    */
    ...
    /*
      ── 소품도 **시트로** 올립니다 ────────────────────────────────────
      «구성» 으로 올릴 때는 고른 인물 시트·**소품 시트**·레퍼런스 영상이 모두 올라가고,
      구도잡기에서 짝지어 둔 것이 `@` 로 정확히 스왑되어야 합니다.
  ...
  const sendCompositionToMagnific = async (
    ...
    /*
      구도를 **쓰기로 한** 컷에서만 구도 그림을 요구합니다 — 구도 없이 인물 시트와 글만으로
      키 이미지를 뽑는 길이 따로 있습니다.
    */
  ...
  /*
    **알아서 켜 주는 실사 기본값** — 화면에도 보여 줍니다.
    ...
    손으로 다 적게 하지 않고 앱이 알아서 채웁니다. 다만 **말없이** 넣지는 않습니다.
  ...
  */
  ...
  const loadShot = async (shot: { label: string; composition: CompositionState }) => {
    ...
    /*
      **깊은 복사로 가져옵니다.**
      ...
      불러온 구도를 고친 것은 그 컷에만 남아야 합니다 — 원본 구도나 다른 컷까지 따라 바뀌면 안 됩니다.
  ...
  /*
    ── 「@ 다시 잇기」 ───────────────────────────────────────────────────
    그림을 넣거나 참고할 그림을 바꿨을 때, 프롬프트에서 **이어진 부분만** 갈아 끼우는 단추입니다.
  ...
  */
   ...
   *
   * 받은 프롬프트는 히스토리에 남기되, **어떤 항목을 켜고 쓴 글인지**까지 함께 적습니다.
   *
  ...
  const keepPrompt = async (
    ...
    /*
      **영문에 한국어가 섞였으면 알려 줍니다.**
      ...
      영문 칸과 한국어 칸이 같은 말을 하고 있는지는 아무도 안 봅니다 — 앱이 봅니다.
  ...
  const applyRulePrompt = async () => {
    ...
    /*
      **켜 둔 토글을 영문 쪽에도 싣습니다.**
      ...
      켜 둔 토글이 프롬프트에 실리는지 훑다 드러났습니다 — 영문 쪽은 안 실리고
    ...
    */
    ...
    /*
      ── 알아서 켜 주는 실사 기본값 ────────────────────────────────────────
      이 앱을 쓰는 사람이 다 프롬프트 전문가는 아닙니다. 인물 질감·카메라 무빙 속도까지
      일일이 정해 놓고 요청하라고 할 수는 없습니다.
  ...
  /* ── 컷을 **영상으로** ────────────────────────────────────────────────
     구도잡기 레퍼런스 영상·인물·배경을 그대로 들고 **컷 영상**까지 갑니다. 러닝타임도
     함께 실어 생성기 쪽에 그대로 앉습니다.
   ...
   *
   * 구도잡기 영상·인물·배경을 한 번에 물려 컷 영상을 뽑습니다 — 미니맥스 H3 의 `ref2va` 가
   * 정확히 그 일을 합니다.
   *
  ...
  /*
    ── 영상 프롬프트를 **LLM 으로** ──────────────────────────────────────
    영상·컷 프롬프트는 상황·배경·환경·인물·구도를 아주 자세히 적어야 합니다. 영상 칸에는 여태 규칙 조립(「영상 프롬프트」)뿐이었습니다 — 규칙은
  ...
  */
   ...
   *
   * 예전에는 원본 생성 이미지 앞 두 장뿐이었습니다 — 넉 장을 뽑아 둔 인물도 둘만 떴고,
   * 에셋(공용인지 인물 것인지)도, 뽑아 둔 시트도 목록에 없었습니다.
   * 원본 전부 + 변형 + 시트 + 보유 에셋 + 공용 에셋을
  ...
  /**
   * ── 인물마다 «이 그림을 레퍼런스로» ────────────────────────────────
   * 인물 스왑에 올릴 그림은 **인물마다 따로** 골라야 합니다 — 한 인물의 시트가 여럿이라, 앱이 아무거나 집으면
   * 그 컷에서 원하는 옷·머리가 아닌 판이 올라갑니다.
   *
  ...
  const toggleCharacterRef = (characterId: string, filePath: string) =>
    patchCut((current) => {
      ...
      /*
        ...
        자동 시트로 물러서서 태그가 그대로였습니다 — 뺐는데 빠지지도, 풀리지도 않았습니다.
        이제 [] 는 「이 컷에서는 그림을 안 쓴다」 입니다(`pickedCharacterRefs`). 자동 시트로 되돌리고 싶으면
        그 그림을 다시 고르면 됩니다.
      */
      const all = { ...(current.characterRefs || {}), [characterId]: next };
      /*
        ── 고르고 나서 «아무 일도 안 일어나는» 것을 막습니다 ────────────────
        손으로 레퍼런스 그림을 잡아도 프롬프트는 그대로였습니다.

        레퍼런스를 고르는 것과 프롬프트에 @태그가 박히는 것은 **다른 일**입니다
        (프롬프트를 통째로 다시 받지 않으려고 «@ 다시 잇기» 로 일부러 나눠 둔 것).
        그런데 화면에는 그 사실이 안 보여서,
        고른 사람은 끝난 줄 압니다.
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-1.5 px-3 py-2.5">
        <button
          ...
          data-tour-switch="cut-frame cut-switches cut-summary cut-refs cut-style-toggles cut-dialogue cut-prompt-section cut-prompt-write cut-video-section cut-video-prompt cut-ref-video-list"
        >
          {open ? (
            <ChevronDown
        ...
        */}
        ...
        {/*
          ── 특수 배경 ──────────────────────────────────────────────
          도면·도면+동선 같은 문구는 구도잡기가 아니라 **컷마다 하나씩** 서는 이 단추로 뺐습니다
          (구도잡기 단추 옆). 컷 프롬프트를 쓸 때 꺼내 쓰는 것이라 그 자리에 있어야 합니다.
        ...
        */}
        {onCreateBackground && onPatchBackground && (
          <button
            ...
            onClick={() => {
              ...
              /*
                공간 유형을 **실내외**로 둡니다. 구성 칸에서 항공뷰·조감도가 통째로 안 보인 적이 있는데,
                칩이 사라진 게 아니라 카드가 «실내» 라 실외 칩(조감도·항공 수직·등각)이
        ...
        {/*
          «이미지 편집»(동선 그리기) 단추는 걷었습니다. 컷에만 있는 기능이 되면 안 되는 것이라,
          이제 그림의 가위(공용 이미지 편집 창)의 «동선» 탭에서 그립니다(규칙 1).
        */}
        <button
          type="button"
          onClick={onRemove}
      ...
      {open && (
        ...
        >
          ...
          {/*
            ── 이 컷을 어떻게 뽑을까 — 스위치 둘 ───────────────────────
            구도는 컷마다 넣고 뺄 수 있고, 뺀 그대로 프롬프트가 달라져야 합니다.
          ...
          */}
                ...
                {imageGroups.map(([ownerName, images]) => {
                  ...
                  /*
                    **안 골랐을 때 실제로 쓰이는 시트**를 찾아 둡니다.
                    ...
                    그림을 뽑아도 인물 레퍼런스에 고른 표시가 없어, 아무것도 안 걸린 것처럼 보였습니다.
          ...
          {/*
            ── 대사 · 연기 지시 ─────────────────────────────────────────
            인물 시트와 레퍼런스 영상으로 스왑해 영상을 뽑을 때, 대사와 연기 지시도 프롬프트에 함께 실립니다.
          ...
          */}
          ...
          {/*
            ── 컷을 영상으로 ───────────────────────────────────────────
            한 컷이 그대로 영상 한 편이 되기도 합니다(그런 씬은 컷이 하나입니다).
      ...
      {/*
        ── 잡아 둔 구도 고르기 ────────────────────────────────────────
        잡아 둔 구도는 목록으로 관리하되, **그림으로** 늘어놓습니다.
      ...
      */}
      ...
      <CompositionPlanner
        ...
        /*
          구도잡기 안에서 **장소 카드**를 만들고 그 자리에서 엽니다 — 방을 세운 흐름 그대로 전개도를 뽑고,
          뽑은 것을 고르면 바로 여섯 면에 걸리게. 카드는 프로젝트의 장소 목록에 그대로 생깁니다 —
          씬 탭에서 보던 그 카드입니다.
        */
        onCreatePlace={
          onCreateBackground
        ...
        /*
          ...
          구도잡기에서 뽑은 영상은 그대로 레퍼런스 영상이 되고, 러닝타임도 함께 실려
          생성기 쪽에 그대로 앉습니다.
        */
