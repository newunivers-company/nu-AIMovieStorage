 ...
 * **작업 판** — 오른쪽에서 열고 닫는 서랍.
 *
 * 무엇이 돌고 무엇이 순서를 기다리는지, 어느 작품의 무슨 일인지, 얼마나 됐는지를 한자리에서
 * 봐야 합니다. LLM 한 번에 수십 초가 걸리고 한참을 도는 일도 있어서, 그동안 「멈춘 건지
 * 진행 중인 건지」 가 안 보이면 더 기다릴지 다시 걸지를 못 정합니다. 그래서 옆으로 열고
 * 닫는 서랍에 내역을 같이 둡니다.
 *
      ...
      {open && createPortal(
        <>
          ...
          >
            ...
            {/*
              한 줄로 줄였습니다. 옆으로 길게 뽑는 서랍인데도 정작 내역이 좁아 보였습니다 —
              설명이 석 줄을 먹으면 그만큼 작업 목록이
...
function TaskRow({ task, now }: { task: QueueTask; now: number }) {
  ...
  /*
    소식이 끊긴 지 오래됐는가. 이것이 「멈춘 건지 진행 중인 건지」 에 답하는
