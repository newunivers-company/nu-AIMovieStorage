 ...
 * 돌려줄 뿐, 나누는 일은 `EntitySheetComposer` 가 합니다. 그래야 다른 인물의 시트를 만들 때도
 * 같은 배치도를 고를 수 있습니다 — 인물마다 칸을 다시 놓으면 시트가 서로 다른 모양이 됩니다.
 *
 * # 상자 크기와 글자 크기는 따로입니다
 *
 ...
 * 그러자 규격을 바꿔도 그림이 비율대로 같이 줄어들 뿐이라, 규격을 바꾸는 의미가 없었습니다.
 * 그림은 **뽑힌 크기 그대로** 들어가야 합니다. 이제 규격 = 캔버스 px, 칸 = 그 캔버스의 px,
...
/**
 * 뽑을 규격 프리셋. 쓰는 생성기마다 받는 크기가 달라, 밑의 가로·세로 입력으로 임의 규격도 됩니다.
 */
...
/** 칸의 최소 px. 이보다 작으면 손잡이를 잡을 수 없고, 칸이 0 으로 수렴해 사라지던 오류가 재발합니다 */
const MIN_SLOT = 200;
...
export default function SheetComposerDialog({
  ...
  ownerName: string;
  /** 넣을 수 있는 그림 전부 — 원본·변형·보유 에셋·공용 에셋. 시트에는 인물뿐 아니라 그가 쥔 물건도 들어갑니다. */
  images: SheetSource[];
   ...
   * 있어야 합니다 — 처음부터 다시 놓는 것은 시트를 새로 만드는 일입니다.
   */
  layouts: SheetLayout[];
   ...
   * 없으면 고칠 때마다 시트가 한 장씩 새로 쌓여 어느 것이 최신인지 알 수 없게 됩니다.
   */
  ...
  */
   ...
   * 남아 굽히면 잘렸습니다. null 이면 치는 중이 아닙니다.
   */
  ...
  /*
    ...
    판은 부르는 쪽이 들고 있고 여기서는 기록만 맡습니다. «손잡이를 눌렀다 그냥 떼면
    판을 쌓지 않는다» 는 규칙은 훅 안에 있습니다. `mark` 는
    ...
    칸만 되돌리면 규격 2048 에 4096 칸이 되살아나 굽히면 잘립니다. 그래서
  ...
  */
   ...
   * 예전에는 긴 변 1800(6000 기준)으로 넣어서 2048 그림이든 1024 그림이든 같은 크기가 됐습니다 —
   * 그러면 시트 위에서 어느 것이 크게 뽑힌 그림인지 알 수가 없습니다. 시트보다 크면 비율을 지켜
   ...
   *
   * 놓인 그림 위에 다른 그림을 끌어다 놓으면 **교체**입니다. 새 칸으로 얹히면 배치가 흐트러지고,
   * 의상만 바꿔 다시 뽑는 흔한 일이 배치를 다시 잡는 일이 됩니다.
   */
  ...
  /**
   * 규격 바꾸기. 칸의 px 는 그대로 두고(그림은 뽑힌 크기로 들어갑니다), 새 시트
  ...
  const bake = async (mode: "new" | "replace") => {
    ...
    try {
      ...
      // «4000 시트 = 6000 시트의 축소판» 이 되어 규격을 바꾸는 의미가 없어집니다.
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
          ...
          <div className="grid min-h-0 flex-1 grid-cols-[240px_minmax(0,1fr)]">
            ...
            >
              ...
              {/*
                ...
                빈 배치도를 더하는 것만으로는 쓸 데가 없습니다 — 이름을 붙여 구분하고,
                다른 인물의 시트를 만들 때 그 배치도를 고를 수 있어야 비로소 값을 합니다.
                그래서 이름은 입력칸이고, 목록은 프로젝트 전체 것입니다.
              */}
              ...
              {/*
                ...
                칸의 px 는 그대로 두고 캔버스만 커지거나 작아집니다(그림은 뽑힌 크기로 들어갑니다).
                고른 규격을 단추·설명·결과에 모두 적어 둡니다 — 어디에도 안 보이던 때는
                실제로는 바뀌었는데도 «안 바뀐다» 로 읽혔습니다.
              */}
              ...
              {/*
                ...
                예전에 숫자를 넣으면 0 으로 수렴하던 오류는 빈 값이나
              ...
              */}
              ...
              {/*
                ...
                나타나던 때는 그런 조절이 아예 없는 것으로 보였습니다.
              */}
              {fontTarget && (
                <div
                  ...
                  return (
                    ...
                    >
                      ...
                      {/*
                        ...
                        처음엔 상자 안 위에 얹혀 그림을 가렸고, 상자 밖 아래로 내렸더니 바로 밑 상자와
                        붙어 어느 칸 이름인지 헷갈렸습니다. 그래서 상자 밖 **위**입니다. 이름을 고칠 수
                        있어야 «전신»·«표정» 처럼 칸의 쓰임을 적어 둘 수 있습니다. 상자가 시트
      ...
      {/*
        ...
        시트를 짜다 보면 «이 칸만 얼굴을 잘라 쓰고 싶다» 가 늘 나옵니다. 창을 닫고 카드로
        돌아가 자른 뒤 다시 열면 배치가 끊겨, 자르기를 여기서 바로 엽니다.
