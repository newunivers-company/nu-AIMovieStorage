        ...
        >
          ...
          {/*
            **IK 이동은 없앴습니다.** 관절 회전만으로 잡습니다.
            ...
            원하는 자리에 못 세웠습니다. 되돌리기도 어려웠고요. 관절을 3축으로
            직접 돌리는 쪽이 만들기도 쉽고, 끌 때마다 결과가 같습니다.

            **뒤에 다시 검토했고 다시 보류했습니다.** Sketch2Pose 처럼 화면에서
            관절을 끌면 3D 가 따라오게 하는 길은 결국 IK 라, 여기서 걷어낸 바로
            그것입니다. 팔꿈치가 제멋대로 따라오던 까닭은 **팔꿈치가 어느 쪽을
            향할지(pole)를 안 정해서** 인 것으로 보이고 그건 고칠 수 있지만,
            다시 들이는 값은 크고 아래 길로 이미 메워집니다.
    ...
    <div data-tour="layout-hands" className="space-y-2">
      ...
      {/*
        **손 모양 프리셋(주먹·가위·보자기…)과 폄/반/접음을 걷어냈습니다.**
        ...
        달라서, 프리셋을 고칠 때마다 다른 손가락이 어긋났어요. 고칠 때마다
        다른 데가 틀어지는 계산이면, 계산을 버리고 손으로 맞춘 값을 저장해
        쓰는 편이 낫습니다.
      ...
      */}
      ...
      {presets.length === 0 ? (
        ...
        <>
          ...
          {/*
            ── 내 프리셋 — 그룹별 버튼 ─────────────────────────────────────
            프리셋은 쓸수록 쌓여서, 그룹으로 나누지 않으면 찾는 데만 한참 걸립니다.
            순서는 끌어서 바꾸고, 지우기 x 는 누를 때 한 번 묻습니다 — 손으로 맞춰 저장한
            값이라 잘못 지우면 다시 만들어야 합니다.
