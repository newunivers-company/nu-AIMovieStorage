 ...
 *
 * `CompositionViewport` 의 마운트 이펙트 안에 함수 열세 개로 흩어져 있던 것을
...
export interface OrbitDeps {
  ...
  scene: {
    ...
    /** 방 껍질 — Shift 로 방을 집을 때만 봅니다(평소에는 앞의 인물·소품이 먼저 잡혀야 합니다). */
    background: {
      rooms: {
        id: string;
...
export function createOrbit(deps: OrbitDeps) {
  ...
  */
   ...
   * 날아갑니다 — 확대해서 파고들수록 화면이 휙휙 도는 것이 이것입니다.
   *
   * ## 바닥은 «너무 멀면» 안 씁니다
   *
   * 지평선 근처를 보면 바닥까지의 거리가 수백 미터로 발산합니다. 그 점을 축으로 잡으면
   * 5m 앞의 인물이 손톱만 한 각도에도 화면을 가로질러 날아갑니다 — 잡아 둔 앵커가 안 먹는 것처럼
   * 보이는 증상이 이것입니다. 화면을 옮기면 지평선이 한가운데로 오기 쉬워 바로 이 일이 납니다.
  ...
  const pickFromEvent = (event: MouseEvent): string | null => {
    ...
    /*
      ── Shift 를 누르고 찍으면 **방** ────────────────────────────────────
      방도 화면에서 집어 단축키로 옮기고 돌리고 키울 수 있어야 합니다. 다만 아무 때나 잡히면 안 됩니다 —
  ...
  /*
    ── 화면에서 바로 집기 ──────────────────────────────────────────────────
    소품·인물을 옆 사이드바 목록에서만 고를 수 있으면, 화면에서 보고 있는 것을 다시 목록에서 찾아야 합니다.
  ...
  */
  ...
  return {
     ...
     *
     * F 는 고른 것을 회전 중심으로 잡습니다. 인물을 고른 채로는 `focus` 가 늘 그 인물을
     * 잡으므로, 고른 것을 풀지 않고도 한가운데로 되돌릴 길이 따로 필요합니다.
     */
