 ...
 *
 * `CompositionPlanner.tsx` 에서 떼어 냈습니다. 목록을 만드는 곳과
...
export function usePlannerAssets({
   ...
   *
   * 소품은 두 길 중 하나로 그려집니다 — 에셋에서 뽑아 둔 시트를 걸어 주거나, 이름만
   * 정해 프롬프트에 싣거나. 이 목록은 앞쪽, 곧 걸 수 있는 시트를 모은 것입니다.
   *
  ...
  /*
    ── 소품 → 에셋 시트 ──────────────────────────────────────────────────
    전개도를 입히고 소품을 놓는 것까지가 한 흐름이고, 그 소품의 에셋 시트를 뽑아 되잇는 것이 그 끝입니다.

    그래서 소품을 놓은 자리에서 에셋 카드를 만들고 **바로 잇습니다**. 이미 이어 둔 것이 있으면 그 카드를 엽니다 —
  ...
  */
   ...
   *
   * 덩어리는 움직일 때도 보일 때도 하나로 다룹니다 — 의자 넷을 «식탁 세트» 로 묶었으면
