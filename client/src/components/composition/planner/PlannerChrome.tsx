 ...
 * 작업 탭. **카메라 탭은 없앴습니다.**
 *
 * 카메라 옵션을 전부 타임라인으로 꺼냈습니다. «앵커를 계속 바라보기» 처럼 탭 안에만 있던
 * 손잡이는 **없는 것과 같습니다** — 오래 쓴 사람도 그런 것이 있는 줄 몰랐습니다.
 *
...
/*
  **탭이 열어 주는 자리들.** 탭 하나가 살아 있는 동안만 그 패널을 그리므로, 다른 탭에 서
  ...
  두면 안내 창이 읽어 한 번 눌러 줍니다(`TutorialOverlay`) — 안내를 따라가다 «탭을 직접
  누르세요» 로 끊기면 그 걸음에서 멈춥니다.
*/
 ...
 *
 * 작품이 정한 비율을 구도잡기 프레임으로 옮기는 데 씁니다 — 이걸 안 옮기면 숏츠 작품인데
 * 구도는 가로 프레임에서 잡게 됩니다.
 */
  ...
  return (
    ...
    >
      ...
      {/*
        ...
        열면 가리킬 자리가 없습니다 — 구도잡기 안내는 구도잡기를 연 다음에야 뜻이 있습니다.
        목록은 «지금 화면» 으로 고르는데, 창이 열려 있는 동안은 그것이
 ...
 * **오른쪽 도구줄 아래로 옮겼습니다**(`PlannerActionBar`).
 ...
 * 그리고 그 아래 **작업 탭 줄**(왼쪽 위에서 옮겨 왔습니다 — 탭이 바꾸는 패널 옆이 손이 짧습니다).
 */
      ...
      */}
      <div className="pointer-events-none flex flex-wrap items-center justify-end gap-1.5">
        ...
        */}
        ...
        {/*
          방 — 한 변이 곧 축척입니다. «배경 고정 · 배경도 함께» 같은 갈래 단추는 걷어냈습니다.
          크기를 바꾸는 자리는 환경 탭 한 곳뿐이라, 여기서는 지금 값만 보여 줍니다 —
          같은 값을 두 자리에서 고치면 어느 쪽이 맞는지 알 수 없습니다.
        */}
        <span
        ...
        {/*
          **«다시 찍기» 를 걷어냈습니다.** «구도 저장» 이 찍으면서 저장하므로,
          이 단추는 «창을 닫지 않고 지금 화면만 다시 넘기기» 라는
  ...
  return (
    <div
      /*
        ...
        머리글과 샷 아이콘을 덮어, 글자가 겹쳐서 카메라 무빙이 안 보였습니다.
        미리보기 중임을 알리는 배지라 화면 쪽이 제자리이기도 합니다.
      */
 ...
 * 하단 — 카메라 조작 도움말.
 *
 * **F 를 반드시 적습니다.** 회전 중심은 눈에 보이지 않아서, 축이 어디로 잡혀 있는지
 * 모르겠다는 막힘은 안내가 없으면 고쳐도 그대로 남습니다.
