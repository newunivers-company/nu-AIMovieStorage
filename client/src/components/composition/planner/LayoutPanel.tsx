...
/*
  ...
  달라 남깁니다 — 목록에 같은 모양이 둘이면 왜 둘인지부터 헷갈립니다.

  아이콘도 모양별로 갈랐습니다 — 셋 다 같은 정육면체 아이콘이라 목록에서 구분이
  안 됐습니다.
*/
/*
  «테이블» 도 뺐습니다 — 상자와 다를 것이 없고, 대신 상자 여럿을 묶어 한 덩어리로
  만드는 길을 두었습니다. 탁자는 «납작한 상자» 라 상자 하나를 눌러 만드는 편이 자유롭고, 다리까지
...
export interface LayoutPanelProps extends SectionToggles {
   ...
   *
   * 소품은 에셋에서 뽑은 것을 짝지어 주거나, 이름만 정해 프롬프트에 실어 보냅니다.
   ...
   *
   * 소품 하나에 에셋 시트 하나가 짝지어져야 그 소품이 무엇인지 프롬프트가 말할 수 있습니다.
   * 안 주면 단추가 안 보입니다(구도잡기를 프로젝트 밖에서 열었을 때).
   */
  onCreateAsset?: (objectId: string, label: string) => void;
  /** 덩어리 쪽 에셋 — 묶음은 묶음 하나로 표시되고, 에셋도 묶음에 하나 붙습니다. */
...
export function LayoutPanel({
  ...
  /**
   * 이어 둔 에셋이 **아직 있는가**. 에셋을 지워도 이음만 남아 «시트 뽑기»·«벽 열기» 가 살아 있었습니다 —
   * 지운 에셋을 가리키는 이음은 «없는 것» 으로 봅니다.
   */
  const linkedAsset =
   ...
   * 여기서 따로 순번을 세다가 화면과 목록의 색이 어긋나, 다른 인물 둘이 같은 색으로 그려졌습니다.
   */
   ...
   *
   * 엑스트라 열 명을 한 색으로 묶는 일이 흔한데, 하나씩 골라 칠하면 열 번을 같은 일에 씁니다.
   */
  ...
  return (
    <>
          ...
          {plannerCharacters.map((character, index) => {
            ...
            return (
              <div key={character.id} className="flex items-center gap-1.5">
                ...
                {/*
                  **마네킹은 목록에서 바로 뺍니다.** 여기서 더한 마네킹은 눈으로 가리는 것만으로는 모자랍니다 —
                  잘못 더한 것이 줄에 계속 남으면 목록이 금방 지저분해집니다.
      ...
      {/*
        ── 고른 인물 ─────────────────────────────────────────────────────
        이름·키·체격은 인물에 딸린 값이라 소품·조명 아래가 아니라 **인물 바로 아래**에 둡니다.
        목록에서 사람을 고르면 그 사람의 칸이 그 자리에서 열립니다.
      */}
          {selectedCharacter && selectedCharacterSource && (
            <>
              ...
              {/*
                ── 몸 색 ────────────────────────────────────────────
                엑스트라는 한 색으로 묶고, 주인공끼리는 반드시 갈려야 합니다.
              ...
              */}
              <div data-tour="layout-body-color" className="mb-3 space-y-1">
                ...
                <div className="flex flex-wrap items-center gap-1">
                  ...
                  {/*
                    고르개로 찍은 색은 **놓는 순간 저장됩니다** — 안 그러면 창을 닫을 때 고른 색이 날아갑니다.
              ...
              {/*
                위치·회전 숫자는 **접어 둡니다**(소품 쪽과 같은 모양으로).
                평소에는 화면에서 끌고, 맞춰야 할 때만 폅니다.
              */}
              <CharacterFields
        ...
        <div className="space-y-1.5">
          {/*
            **캐릭터 쪽 소품만** 섭니다. 방에 딸린 배경 소품은 환경 탭의 그 방 속성에 있습니다
            — 한데 섞어 두면 배치 탭에서 놓은 소품이 환경 탭 목록에도 그대로 나옵니다.
      ...
      {/*
        ...
        — 기즈모 단추와 그 값이 떨어져 있으면 무엇을 고치는 숫자인지 짚기 어렵습니다.
      */}
      {(selectedCharacter || selectedObject) && (
        ...
        >
          {/*
            ── 고른 덩어리 ───────────────────────────────────────────────
            덩어리를 고르면 남는 것은 **덩어리에 시트 뽑기**뿐입니다 — 낱개 소품의 이름 칸이
            같이 떠 있으면 어느 쪽 이름을 고치는 것인지 알 수 없습니다.
          ...
          */}
          ...
          {selectedObject && !selectedGroup && (
            <>
              ...
              {/*
                ── 고른 소품의 수치 ───────────────────────────────────
                수치 입력·색·«모두 초기화» 는 기즈모 단추 **바로 아래**입니다 — 고치는 손잡이와
                그 값이 붙어 있어야 어느 소품의 숫자인지 헷갈리지 않습니다.
              */}
              {/*
                수치 입력과 색은 **접어 둡니다.** 평소에는 단축키와 마우스로 하니까요(캐릭터 탭도 같습니다).
                환경 탭과 **같은 칸**입니다.
              */}
              <ObjectFields
              ...
              {/*
                **벽과 조명에는 에셋이 없습니다.** 에셋을 뽑는 것은 상자·구·실린더뿐 —
                벽은 그림을 입히는 것이고, 조명은 그려지는 물건이 아닙니다.
              */}
              ...
              >
                ...
                {/*
                  **에셋만** 뜹니다. 소품을 «인물 시트» 로 바꿔 그릴 일은 없으니, 인물 그림까지
                  목록에 섞으면 고를 것만 길어집니다 — 인물은 인물 자리에 섭니다.
                */}
                {swapOptions
              ...
              {/*
                ── 여기서 **에셋을 만들어 바로 잇습니다** ────────────────
                전개도를 입히고 소품을 놓았으면, 다음 걸음은 그 소품의 에셋 시트를 뽑아 짝짓는 일입니다.
              ...
              */}
              ...
              {/*
                ── 인물 관절에 붙이기 ─────────────────────────────────────
                소품의 기준점을 인물의 한 관절에 매답니다. 검을 쥔 손처럼, 자세가 바뀌면
                물건도 따라 움직여야 하는 자리가 있습니다.
              ...
              */}
              ...
              {/*
                ── 이 벽의 그림 ──────────────────────────────────────
                벽을 고르면 그 벽의 그림은 소품 속성 **아래**에 붙습니다 —
                자주 만지는 수치·에셋이 위, 한 번 고르면 되는 그림이 아래입니다.
              */}
              {/*
                ── 벽에 붙일 그림 ─────────────────────────────────────
                벽을 손으로 세우고 그 벽에 그림을 입혔다 바꿉니다. 그림은 **벽 크기에 맞춰** 뽑고,
                인물과 벽 사이의 거리도 재어 함께 보냅니다 — 그래야 원근이 화면과 맞습니다.
              ...
              */}
              ...
              {/*
                  ── 어떤 시트로 바꿀까 ────────────────────────────────
                  소품은 에셋에서 뽑은 시트를 짝짓거나 이름만 프롬프트에 실어 보냅니다.
                  짝짓는 자리는 소품을 놓는 **배치 탭**입니다.
      ...
      {selectedCharacter && (
        ...
        >
          {/*
            ...
            2026-09-11 에 걷어냈습니다 — 나오는 자세가 어색한 데다, 어차피 프리셋을 저장할 수
            있으니 필요할 때 직접 만들어 두는 편이 낫습니다. 자리는 옛 포즈 목록 그대로입니다.
