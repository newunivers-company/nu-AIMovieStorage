 ...
 *
 * 발 클로즈업인데 앵커가 가슴에 붙으면 축이 엉뚱한 데 섭니다. 클로즈업은
 * 무엇을 크게 잡느냐가 전부라, 축도 그 자리에 서야 합니다.
 */
/**
 * 키를 찍을 수 있는 속성. 색이 **줄마다 다릅니다.**
 *
 * 한 키가 무엇에 대한 키인지 보이지 않았습니다. 줄이 둘만
 ...
 *
 * 편집 프로그램처럼 구도잡기 아래에 두고 «0~2초 달리 인, 2~3초 팬 좌» 식으로 이어 붙여
 * 카메라 무빙을 짓습니다.
 *
 * # 왜 «레이어» 가 아니라 «줄» 인가
 *
...
export function PlannerMoveTimeline({
   ...
   *
   * 이미 들어간 모션은 줄을 골라 오른쪽 단추로 다른 모션으로 바꾸거나 다시 분석할 수
   * 있어야 합니다 — 지우고 다시 넣으면 자리와 길이를 처음부터 맞춰야 합니다.
   *
  ...
  /*
    ── A 키 — 따라갈 앵커 레이어 넘기기 ─────────────────────────────────
    줄을 고르고 A 를 누르면 따라갈 앵커의 줄이 넘어갑니다. 목록을 열어 고르는 것보다
    눌러서 넘기는 쪽이 빠릅니다 —
  ...
  */
  ...
  /*
    ── 접고 펼 때 **아래로 스르르** ─────────────────────────────────────────
    Ctrl+Space 로 접고 펼 때 아래로 내려가며 접히고 위로 올라오며 펴집니다 — 툭 사라지면
    판이 닫힌 것인지 화면이 깨진 것인지 알 수 없습니다.
  ...
  */
   ...
   *
   * 카메라 클립과 인물·소품 트랙이 **한 자리**를 나눠 씁니다 — 인물의 이동·회전에도
   * 같은 속도 그래프가 걸려야 합니다. 둘을 따로 두면 화면에
   * 그래프가 둘 뜨고, 안 그래도 좁은 세로가 더 좁아집니다.
   *
   * # 왜 «떠 있는 판» 인가
   *
   * 처음에는 타임라인 안에 그대로 끼워 넣었습니다. 그랬더니 그래프 높이만큼 타임라인이
   * 자라 **구도잡기 머리말까지 밀고 올라가 겹쳤습니다.**
   * 타임라인은 화면 아래에 떠 있는 판이라 자랄 자리가 없습니다.
   ...
   *
   * 키마다 카메라 자리·회전·줌을 숫자로도 넣을 수 있어야 합니다.
   ...
   * 고른 키. Delete 로 지웁니다.
   *
   * 키는 잡아서 옮기고, 지우는 것은 Delete 가 맡습니다.
   ...
   *
   * 고무줄로 훑어 여러 키를 한 번에 잡고 한 번에 옮깁니다 — 춤처럼 키가 빽빽한 것은
   * 하나씩 옮겨서는 끝이 안 납니다.
   *
   * 처음에는 «한 사람의 한 채널» 로 좁혔는데, 줄마다 따로 잡히는 것이 틀렸습니다.
   * 한 사람의 이동·회전·자세는 **같은 순간의 한 동작**
  ...
  /*
    고른 **클립**도 Delete 로 지웁니다 — 이미 배치한 카메라 무빙도 같은 키로
    지웁니다. 키를 고른 상태면 키가 먼저입니다 — 방금 만진 것을
    지우는 쪽이 덜 놀랍습니다.
  */
  useEffect(() => {
  ...
  const dragKey =
    ...
    (event: React.PointerEvent) => {
      ...
      /*
        ── 고르면 **바로** 값이 열립니다 ─────────────────────────────────
        키를 고르면 그 값이 곧바로 열려야 합니다 — 예전에는 두 번 눌러야 했습니다.
  ...
  const dragJoint =
    ...
    (event: React.PointerEvent) => {
      ...
      const up = () => {
        ...
        /*
          누르기만 했으면 **그 시각으로 가서 그 관절을 잡습니다.** 확인하고 그 자리에서
          바로 고치라고 — 기즈모로 돌리면 자동 키가 이 자세 키에 그대로 들어갑니다.
        */
        onSeek(key.time);
   ...
   *
   * 모캡을 넣은 채 전체 러닝타임을 줄이면 찍어 둔 키가 빠진 것처럼 보였습니다. 러닝타임은
   * 언제든 바뀝니다 — 노래보다 짧은 영상을 만드는 일도 흔합니다.
   *
  ...
  */
  ...
  /*
    ── 접힌 타임라인 ──────────────────────────────────────────────────────
    판이 길어지면 3D 화면이 가려집니다. 아래로 접는 단추와 Ctrl+Space 를 둡니다.
  ...
  */
    ...
    return shell(
      ...
      >
        ...
        {/*
          ...
          한 번 누릅니다) — 튜토리얼은 스스로 열고 가리켜야 합니다.
        */}
        <button
          type="button"
  ...
  return shell(
    <div
      /*
        재생 중에 타임라인 **어디를 눌러도** 멈춥니다 — 누르든 스페이스를 치든 결과가
        같아야 합니다.
    ...
    >
      ...
      {/*
        ── 자유 경로 키의 값 ────────────────────────────────────────────
        키마다 카메라 자리·회전·줌을 손으로도 넣을 수 있어야 합니다.
      ...
      */}
      ...
      <div className="mb-1 flex items-center gap-2">
        ...
        {/*
          ── 무빙이 어디서 출발하는가 ────────────────────────────────────
          무빙의 시작점이 구도 1 의 카메라라면 실제로 그 카메라에서 출발해야 합니다 —
          아니면 구도를 저장해 둔 뜻이 없어집니다.
        ...
        */}
        ...
        {/*
          재생 단추. 무빙을 고치면서 바로 돌려 봐야 합니다 — 고치는 자리와 확인하는 자리가 달라서
          타임라인 탭까지 가야 했습니다.
        */}
        {/*
          ── 키로 건너뛰기 ────────────────────────────────────────────────
          키가 찍힌 자리로 곧바로 건너뛰는 단추 — 그래야 그 키에 이동값·회전값을 더 넣습니다.
        ...
        */}
        ...
        {/*
          ── 앵커를 여기서 바로 ──────────────────────────────────────────
          앵커는 무빙을 고치는 이 자리에서 바로 잡습니다. 손으로 좌표를 옮길 수도 있고,
          인물의 한 부위나 소품에 붙일 수도 있어야 합니다.
        ...
        */}
        {/*
          ── 오른쪽 묶음 ────────────────────────────────────────────────
          키·이동·회전·크기 단추는 카메라 무빙을 넣었을 때와 같은 오른쪽 자리에 섭니다.
        ...
        */}
        <span className="ml-auto flex items-center gap-1">
          {/*
            ── 앵커는 늘 보입니다 ────────────────────────────────────────
            앵커는 대상을 안 골랐어도 자유롭게 바꿀 수 있어야 하고, 대상을 고르면
            그때 거기에 붙일 수 있어야 합니다.
          ...
          */}
          {selectedMove && (
            ...
            >
              ...
              {/*
                손 단추 옆의 앵커 좌표 칸은 단추와 높이를 맞춥니다. 공용 `NumberInput` 은
                패널용이라 이 줄에서는
              ...
              */}
              ...
              {/*
              어느 높이에 붙일지 고릅니다 — 머리에도 발에도 붙습니다. 발 클로즈업인데
              앵커가 가슴에 있으면 축이 엉뚱한 데 섭니다.
            ...
            */}
              ...
              {/*
              ── 세 손잡이를 여기로 ────────────────────────────────────────
              카메라 탭으로 건너가서 고쳐야 한다면 무빙을 타임라인으로 옮긴 뜻이
              없습니다 — 카메라 탭에서 하던 일을 여기서 다 끝냅니다.
            ...
            */}
              ...
              {/*
              ── 어느 클립의 앵커를 따라갈까 ──────────────────────────────
              클립마다 따라갈 앵커를 따로 고릅니다 — 달리 인에서 잡은 앵커를 오빗 90 이
              그대로 따라갈지, 클립마다 다른 앵커를 볼지가 여기서 갈립니다. 줄을 고르고
              A 로 넘깁니다.
      ...
      {/*
        ── 샷 고르기 ────────────────────────────────────────────────────
        카메라 탭에 있는 도식을 그대로, 타임라인 위에 가로로 길게 늘어놓습니다.
      ...
      */}
        ...
        {SHOT_PRESETS.filter((preset) => !preset.hidden).map((preset, index) => (
          <button
            ...
            /*
              ...
              적어 둡니다 — 안 그러면 속도 그래프가 어디 있는지 가리킬 자리가 없습니다.
            */
            data-tour-open={
            ...
            /*
              ...
              위의 무빙 아이콘을 끌어다 클립에 놓으면 그 줄의 무빙이 바뀝니다.
            ...
            */
            ...
            onClick={() => {
              /*
                ── 카메라를 먼저 세우고 무빙 ─────────────────────────────
                저장한 구도가 없으면 무빙을 놓지 못하게 막습니다. 구도를 고른 뒤에 놓고,
                놓은 뒤에는 다른 구도로 바꿀 수 있습니다.
            ...
            style={{
              // 아이콘이 작아 무슨 무빙인지 안 읽혔습니다 — 46 → 62px.
              width: 62,
              /*
                앵커를 쓰는 무빙(오빗·달리)만 노란 기가 돕니다. 팬·틸트·트럭은 앵커와
                상관이 없어서, 앵커를 옮겨도 그림이 안 바뀌는데 단추가 똑같이
                보이면 「왜 안 먹지」 가 됩니다.
              */
      ...
      {/*
        ...
        재생선이 눈에 띄지 않았습니다. 예전 선은 눈금자 12px 안의 1px 선이라, 아래
      ...
      */}
      {/*
        ── 확대·축소 ────────────────────────────────────────────────────
        +·− 로 타임라인을 늘이고 줄입니다 — 모캡 영상은 키가 빽빽해서, 그대로는 집어
        고칠 수가 없습니다.
      ...
      */}
      <div className="flex items-center">
        {/*
          ...
          총 길이와 프레임 수를 손으로 넣습니다. 5초는 기본값이었지 한계가 아니었는데,
        ...
        */}
        ...
        >
          ...
          {/*
            ...
            (`usePlannerPlayback`) 자리를 **ref 로 직접** 씁니다 — 재생 중에도 시간이
            흘러가는 것이 보여야 합니다.
          */}
        </div>
      </div>
      ...
      {/*
        ── 고른 클립 손잡이 ─────────────────────────────────────────────
        속도 그래프는 키를 고른 그 자리에서 바로 열어 걸고 확인합니다.
      ...
      */}
      {selectedMove && (
        <div className="mt-1 flex items-center gap-1">
          ...
          {/*
            ...
            달리 인·틸트 다운 같은 프리셋 무빙은 «어디서 시작해 얼마나 가는가» 가 전부라
            키가 따로 필요 없습니다 — 이동량과 걸리는 시간, 그 사이의 속도 곡선이면 됩니다.
            «오빗 90도» 도 끝 값을 45 로 바꾸면 그만입니다.

            프리셋 클립은 **«지금 자세에서 얼마나»** 라 절대 좌표가 없습니다.
          ...
          */}
          ...
          {/*
            고른 **이동량 키**의 값 — 키를 고르면 그 수치가 곧바로 열립니다.
            클립 전체의 이동량 옆에 나란히 두어, 「이 클립은 −2.5까지
          ...
          */}
          ...
          {/*
            ── 카메라 탭에 있던 나머지 손잡이 ───────────────────────────
            앵커 계속 바라보기·회전 축·손떨림까지, 카메라 탭에 숨어 있던 손잡이를
            전부 여기로 꺼냅니다.

            탭에 숨어 있던 손잡이는 **없는 것과 같습니다** — 있는 줄도 모르고 지나갑니다.
            무빙을 고른 자리에 다 모아 둡니다.
          */}
          ...
          {/*
            ── 손떨림 ──────────────────────────────────────────────────
            손떨림은 슬라이더 말고 숫자로도 넣습니다. 기준값을 같이 적어야
            «핸드헬드는 몇 %인가» 를 알 수 있습니다.
      ...
      {/*
        ...
        **높이를 묶지 않습니다.** 예전에는 클립이 늘면 판이 3D 화면을 밀어 올려서 96px 칸에
        ...
        그 좁은 칸 안에서 레이어를 스크롤하며 작업하게 됐습니다 — 길어지는 것을 접기로
        푸는 판에 스크롤까지 넣으면 작업만 어려워집니다. 펼치면 예전 그 화면 그대로 나오고,
        길이는 «접기»(Ctrl+Space) 가 맡습니다.
      ...
      */}
      ...
      >
          ...
          return (
            <div key={move.id}>
              <div className="flex items-center">
                ...
                무빙 줄마다 출발할 저장 구도를 따로 답니다 — 카메라 1 로 무빙을 주다가
                카메라 2 로 구도를 바꿔 이어 갈 수 있어야 합니다.
              ...
              */}
                ...
                >
                  ...
                  {/*
                  줄 차례 바꾸기. «이어서» 는 위 줄에 이어붙는다는 뜻이라 차례를
                  바꿀 수 있어야 합니다 — 줄은 시각
                ...
                */}
                ...
                <div className="relative h-5 flex-1 overflow-hidden">
                  <div
                    ...
                    onDoubleClick={async () => {
                      /*
                    클립을 두 번 누르면 길이를 숫자로 고칩니다. 끝을 끄는 것보다
                    숫자가 빠른 때가 있습니다 —
                    「정확히 2초」 같은 값.
                  */
                      const seconds = await promptNumber({
                ...
                */}
                    {/*
                  이동량 키 — 「이 시각에 몇 도」. «오빗 90도» 라고 해서 반드시 0~90도를
                  돌아야 하는 것은 아닙니다. 두 번 누르면 값을 고칩니다.
                */}
                    {preset?.kind !== "free" &&
                      sortByTime(move.amountKeys ?? []).map((key) => {
                        ...
                        return (
                          /*
                          ── 키가 먼저 ───────────────────────────────────
                          키를 옮기려는데 클립 전체가 따라 움직였습니다. 키가 있는
                          자리에서는 키가 먼저입니다.
                    ...
                    {/*
                    ── 끝 손잡이 ─────────────────────────────────────────
                    지우기는 Delete 가 맡으니 x 단추는 필요 없습니다. 끝자리는 길이를
                    늘이고 줄이는 데 써야 합니다.
              ...
              {/*
                ── 자유 경로의 키는 **줄 아래로** ──────────────────────────
                키는 소품처럼 줄 아래로 내려 정리합니다 — 막대 위에 얹혀 있으면 잡기가
                어렵고, 요소마다 제 키를 찍을 자리도 없습니다.
        ...
        {/*
          ── 인물·소품 레이어 ──────────────────────────────────────────────
          시작 자리·끝 자리를 단추로 뺄 까닭이 없습니다 — 카메라처럼 막대를 깔고 그 아래
          키를 찍으면 디자인도 한 모양이 되고, 인물이 언제 나타나 언제 사라지는지가 막대의
          양 끝으로 바로 보입니다. 편집 프로그램이 다 그렇게 합니다.
        ...
        */}
        ...
        {(layerTargets ?? []).map((target) => {
          ...
          /*
            ...
            처음엔 «키가 있거나 고른 대상» 이면 저절로 폈는데, 속성이 많아 줄이 금세
            길어집니다. 대상을 고르고 옮기거나 돌리면 키는 알아서 들어가고, 키는 K 로
            ...
            <div key={target.id}>
              <div className="flex items-center">
                <div className="flex shrink-0 items-center gap-0.5" style={gutterStyle}>
                  ...
                  {/*
                    ── 속도 그래프는 **레이어에** 하나 ─────────────────────────
                    속도 그래프와 눈 단추는 속성이 아니라 **레이어**에 걸리는 스위치라 레이어 쪽에
                    둡니다. 그래프는 **고른 키의 구간**을 고칩니다(`graphCurve` ①) — 이동 키를
                  ...
                  */}
                  ...
                  >
                    {target.name}
                    {/*
                      **어떤 모션이 들어갔는지** 줄 이름 옆 괄호에 적고, 모션을 바꾸면 그
                      이름도 따라 바뀝니다. 키만 보고는
                  ...
                  return (
                    ...
                    <div className="flex items-center">
                      ...
                      >
                        ...
                        {/*
                          ── 자세 줄의 점 = 관절 점의 **요약** ─────────────────────────
                          펼쳤을 때는 자세 줄에 키를 안 두고, 접었을 때만 그 자리에 점을 둡니다 —
                          접힌 점 하나가 그 순간의 관절 키 전부를 대신해야 한 번에 옮길 수
                          있습니다.
                    ...
                    {/*
                      ── 관절 줄 ─────────────────────────────────────────────────
                      자세 줄을 펴면 어느 관절이 어떻게 움직였는지 보고 고칩니다.
