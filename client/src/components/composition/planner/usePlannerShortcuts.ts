 ...
 *
 * `CompositionPlanner.tsx` 에서 떼어 낸 것입니다. 셋 다 «창이 열려 있을 때만
...
export function usePlannerShortcuts({
  ...
  /*
    ...
    K 가 따로 있는 까닭은 **값이 안 바뀌는 키**입니다. 자동 키는 값이 달라져야 찍히니,
    1초의 자세를 3초까지 그대로 붙잡아 두고 4초부터 움직이고 싶을 때가 찍을 길이 없습니다.
    그 자리를 손으로 찍는 것이 K 입니다.
  ...
  */
  useEffect(() => {
    ...
    const onKey = (event: KeyboardEvent) => {
      ...
      /*
        ── 자세 키 ──────────────────────────────────────────────────────
        자세도 키로 찍습니다 — 포즈 프리셋을 입힌 뒤에도 그 위에 모션이 얹힐 수 있어서,
        «지금 이 자세» 를 시각에 못 박아 둘 자리가 필요합니다.
    ...
    const onKey = (event: KeyboardEvent) => {
      ...
      /*
        **단추에 초점이 있어도** 받습니다. 예전엔 단추를 걸렀는데, 타임라인 레이어 이름·키 단추를
        한 번 누르면 초점이 거기 남아 스페이스가 재생을 못 멈췄습니다. 마우스로 누르든 스페이스를
        누르든 재생은 멈춰야 합니다 — 초점을 풀어 그 단추가 스페이스로 눌리지 않게 합니다.
      */
