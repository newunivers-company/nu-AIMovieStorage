...
/*
  ── 네 단계 ───────────────────────────────────────────────────────────────
  배경을 따로 된 탭으로 두지 않습니다 — 구도를 잡는 그 자리에서 배경과 배경 에셋을 만들어 바로 이어야
  손이 끊기지 않습니다.
...
export default function NewProjectPage() {
   ...
   *
   * 작업실 페이지는 앞 단계와 내용이 겹쳐 통째로 걷어냈습니다 — 갈 곳은 프로젝트
   * 목록뿐이라 단추도 하나면 됩니다.
   */
  const finish = () => {
    if (!commit()) {
   ...
   *
   * 두 창 사고의 뒷면입니다. 다른 창이 먼저 저장해 되읽었을 때, 되읽은 것을
  ...
  */
   ...
   * 뽑기 일들이 읽을 때**. 뒤엣것이 이번에 생겼습니다 — 자동으로 돌렸는데 나온 것이
   * 하나도 없고 작업 서랍에 「찾지 못했습니다」 가 스물네 개 쌓여 있었습니다.
  ...
  */
  ...
  const syncFolders = (target: ProjectDraft, id: string) => {
    ...
    // 첫 그림이 날아올 때 받을 자리가 없으면 그림이 갈 데를 잃습니다.
    ...
    void (async () => {
      try {
        ...
        // 한 번 더 묻고, 취소하면 이름이 되돌아가 폴더와 어긋나는 핑퐁이 됩니다.
        if (again) {
          const base = settled
        ...
        <main className={`${WORK_WIDTH} pb-16 pt-5`}>
          ...
          */}
          <div data-tour="project-progress" className="mb-5 flex items-center gap-0">
            ...
            {/*
              **«저장» 단추는 걷어냈습니다.** 여기서 하는 일은 전부 저절로 저장됩니다 —
