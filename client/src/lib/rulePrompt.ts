...
export interface RulePromptInput {
   ...
   *
   * 기본 정보가
   * **한국어 프롬프트에만** 실리고 있었습니다. 「나이 19 · 키 162cm · 마른 편」 이 영문에는
...
export function buildRulePrompt(input: RulePromptInput): RulePromptResult {
  ...
  */
  ...
  if (unfoldChip) {
    /*
      ...
      옆면이 항공 시점으로 끌려갔습니다(2026-09-16 MCP 실측, `usePromptCard.withUnfoldFrame` 주석). 장소는 사람이 적은 설명과
  ...
  */
  ...
  /*
    ...
    그 부분을 통째로 무시하거나 글자로 그려 넣습니다(2026-09-18 점검). 옛 카드는 영어판이
