 ...
 *
 * 실제로 두 번 났습니다. 배경 프롬프트 규칙을 고쳤는데 며칠 동안 안 먹었고,
 * 컷 대사·연출 칸을 붙였을 때는 며칠 묵은 사본에 가려 **한 컷도 안 들어왔습니다.**
...
export const IMAGE_MODELS: {
  ...
  */
  ...
  // 옮겼습니다 — 모델 칸과 플랫폼 칸에 같은 이름이 둘 다 있으면 무엇을 고른 건지 알 수 없습니다.
  //
...
export function normalizeModelId(id?: string) {
  ...
  /*
    ...
    플럭스·스테이블 디퓨전을 목록에서 뺐으므로, 예전에 그걸 골라 둔 카드가 빈 칸이 되면
    «모델 없음» 으로 프롬프트를 지어 버립니다.
  */
 ...
 *
 * 고르지 않아도 알아서 잘 써 주는 것이 중요합니다. 기법 문서는 칩을 골라야 붙는데,
 ...
 * 조용히 아무 일도 안 합니다** — 오류도 안 나고 파일도 안 생겨, 누르는 쪽에서는
 * 단추가 고장 난 것으로 보입니다.
 *
