 ...
 * `CutCard.tsx` 에서 떼어 낸 것입니다 — 화면 요소를 하나도 안 쓰는 순수
 * 문자열 조립인데, 한·영 삼항이 열한 번 되풀이되어 카드 본문을 가장 크게 불리고 있었습니다.
 *
 * # 안내문은 «누른 칸의 말» 로 씁니다
 *
 * 한글 칸에서 «구성» 을 눌렀는데 보낸 것이 영문으로 보이던 일이 있었습니다.
...
export interface ComposeLinesInput {
   ...
   * 배치도의 색 덩어리 ↔ 진짜 물건.
   *
   * `tag` 는 구도잡기에서 그 소품에 **이어 둔 에셋 시트**의 `@태그`입니다(`swapRef`). 인물뿐 아니라
   * **소품도 시트를 올려 @로 갈아 끼워야** 합니다 —
...
export function cutComposeLines(input: ComposeLinesInput): string[] {
  ...
  /*
    ...
    줄마다 덧붙였는데, 군더더기였습니다. 본문의 문장이 곧 «이 사람을 여기 이렇게» 라,
    거기 칩이 서 있으면 됩니다.
  */
  ...
  /*
    ...
    실측: 캐릭터 시트를 레퍼런스로 올렸더니 생성기가 시트를 «그려야 할
