 ...
 *
 * 일괄 생성이 도는 중에 빠져나와 다른 작품을 만들 수 있어야 하고, 돌아오면 작품마다
 * 제 것이 그대로 있어야 합니다.
 *
 * # 왜 React 상태로는 안 되는가
 *
 ...
 * 한 파일에 살림과 절차가 같이 있어 700줄에 다다르자 둘로 갈랐습니다.
...
export interface BootstrapInput {
   ...
   * 넣고 **이미지·영상까지 바로 뽑을까.**
   *
   * 켜 두고 만들면 글·프롬프트·구도까지 넣고 그림과 영상까지 이어서 뽑습니다. 끄면
   * 글·프롬프트·구도만 세팅하고 멈춥니다.
   *
   ...
   *
   * 일괄 생성으로 받은 프롬프트가 상황·환경·동작·표정 묘사 없이 부실하면 API 를 쓴 뜻이 없습니다.
   *
   ...
   * 「비우고 새로」 를 **미리 확인받았는가.**
   *
   * «적용» 단추는 두지 않습니다. 받은 뒤에 «적용» 을 기다리면, 답이 다 나왔는데 사람이
   * 그 자리에 없어서 아무 일도 안 일어납니다 — 그림도 영상도 안 뽑히는 것처럼 보이던 까닭이
   * 이것이었습니다. 그래서 지우는 일에 대한 확인은 **만들기를 누를 때** 받고, 받은 답은 늘
   * 저절로 들어갑니다.
   */
  replaceOk?: boolean;
  /**
   ...
   *
   * 영상이나 그림을 올려 «이 영상에서 무엇을 참조해 어떻게 만들라» 고 시킬 수 있어야 합니다.
   * 대표적인 쓰임이 **댄스 커버**입니다 — 춤 영상을 올리고 동작은 그대로, 인물과 배경만 갈아
   * 끼웁니다.
   *
   ...
   *
   * 시나리오·기획안을 PDF·워드 파일째로 올릴 수 있고, 원본은 프로젝트 폴더 안 `DOCU` 로 들어갑니다.
   *
 ...
 *
 * 왜 필요한가 — 파일을 여러 장 올리면 «인물은 어떤 그림이고 동작은 어떤 영상인지»
 ...
 * 표기를 쓰면 배울 것이 둘이 됩니다. 그래서 여기도 `@` 를 붙이고, 이어진 것은 글자 색으로
 * 알립니다.
 *
...
/** 지난 답 한 벌. 값을 치르고 API 로 받은 자료라 버리지 않습니다. */
...
export interface BootstrapRun {
  ...
  /**
   * 0 이면 쉬는 중. 1·2·3 은 몇 번째 왕복인가. 4 는 **카드마다 프롬프트를 쓰는 중** —
   ...
   *
   * 프롬프트 카드가 오래전부터 하던 일과 같습니다(`promptHistory`) — 두 번 돌렸다는 것은 무언가를
...
export function adoptBootstrapRun(from: string, to: string) {
  ...
  /*
    **줄에 선 일도 새 열쇠로.** 세 왕복(`bootstrap`)이 아직 차례를 기다리거나, 4단계(`bootstrapPrompt`)가 카드마다
    ...
    실패했습니다. 열쇠를 바꾸는 곳이 여기 하나라 옮기는 것도 여기서 한 번에(`retargetTasks`).
  */
...
/*
  ...
  받은 답은 **저절로 들어갑니다.** 사람이 창 앞에 앉아 있어야만 반영되면, 십 분짜리 답이
  자리를 비운 사이에 그냥 놓입니다.
...
export function restoreBootstrap(project: string, historyId: string) {
  ...
  // 알림은 **저장 결말을 본 뒤** — 닫힌 작품이면 거절될 수 있습니다(두 창을 함께 열었던 사고).
 ...
 * `writeProject` 가 결말을 낼 때 그것을 «들어갔다» 로 돌려줍니다(예전에는 닫힌 길이 실패하면
 * 화면이 열려 있는데도 «열면 들어갑니다» 만 남아, 닫았다 열 때 두 벌이 됐습니다).
 */
...
export function deliver(project: string, announce = false) {
  ...
  */
  ...
  /*
    ...
    ── 넣고 **바로 뽑기** ────────────────────────────────────────────────
    켜 두었으면 넣은 그 길로 그림·영상까지 줄에 세웁니다 — 돌려 놓고 다른 일을 할 수 있어야
    «한 번에» 입니다.
    ...
    ── 왜 `done()` 이 결말 뒤인가 ──────────────────────────────────────────
    닫힌 작품에 부을 때 저장이 «다른 창이 먼저 썼다» 로 거절돼도 예전에는
