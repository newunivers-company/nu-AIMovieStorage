...
export interface CharacterComposition {
   ...
   *
   * 색을 직접 고를 수 있어야 합니다 — 캡처에는 이름표가 안 나가고 프롬프트가 「파란 사람은 @…」
   * 로 짝을 맞추므로, 두 사람의 색이 겹치면 생성기가 인물을 통째로 바꿔 그립니다.
   *
...
export interface CompositionMannequin {
   ...
   *
   * 주·조연과 달라야 할 때도, 엑스트라 여럿을 **일부러 같은 색**으로 묶고 싶을 때도 있습니다.
   *
 ...
 * 소품의 모양.
 *
 * `table` 은 목록에서 뺐습니다 — 탁자는 결국 «납작한 상자» 라 상자와 다를 것이 없고,
 * 상자를 눌러 만들고 여럿을 묶는 편이 훨씬 자유롭습니다.
 ...
 *
 * 상자·구를 여러 개 쌓아 모양을 만든 뒤 하나로 묶고, 묶음에 색을 입혀 «노란 구 덩어리는
 * 안개» 처럼 프롬프트에서 짚어 줍니다.
 *
 * # 왜 «색» 이 핵심인가
 *
...
export interface CompositionObjectGroup {
   ...
   *
   * 묶은 것을 또 묶을 수 있어야 합니다 — 상자 넷으로 의자를 만들고,
   ...
   * 덩어리째 인물의 관절에 매달기.
   *
   * 묶음째 붙여도 **모양이 흐트러지면 안 됩니다** — 낱개로 붙이면 관절마다 제각각 끌려갑니다.
   *
 ...
 *
 * 방 안의 소품은 어딘가에는 닿아 있습니다 — 바닥에 놓이거나 벽에 걸리거나 천장에 매답니다.
 * 방 크기를 바꿀 때마다 그것들을 손으로 다시 맞추는 것은 할 일이 아닙니다.
 *
...
export interface ObjectComposition {
   ...
   *
   * 인물이 든 물건은 포즈를 바꿀 때마다 손을 따라와야 합니다 — 검을 쥔 손을 들었는데
   * 검이 제자리에 있으면 자세를 다시 잡을 때마다 물건도 다시 놓아야 합니다.
   *
   ...
   *
   * 배치 탭의 소품과 환경 탭의 소품은 따로 관리합니다 — 한 목록에 섞으면 길어져 찾을 수 없고,
   * 인물이 쥔 물건이 방을 라이브러리에 담을 때 같이 딸려 갈 이유도 없습니다.
   *
   ...
   *
   * 벽만 투시되고 소품은 안 되면 방 안이 여전히 안 보입니다 — 거꾸로도 마찬가지입니다.
   ...
   *
   * 여섯 면을 다 갖춘 방을 세우는 대신 **보이는 쪽만** 세우는 길입니다. 고정 샷이면 정면과 바닥,
   * 클로즈업이면 정면 한 장이면 됩니다. 한 컷에 필요한 것은 대개 정면 한 장이라, 그 한 장을
 ...
 * 파노라마가 목록에 하나도 안 떴습니다 — 뽑아 둔 그림이 통째로 사라진 것처럼 보입니다.
 *
...
export interface CompositionRoom {
   ...
   *
   * 방을 세울 때 실내·실외를 고르게 한 까닭 — «이미지 생성» 이 전개도를 뽑을지 파노라마를 뽑을지가 여기서 갈립니다.
   */
  outdoor?: boolean;
  /**
   ...
   *
   * 슬라이더를 끌어 본 뒤 원래 값으로 돌아갈 단추가 방마다 있어야 합니다.
   *
   ...
   *
   * 실외 전개도의 옆면을 «지평선이 벽 맨 아래 3 %» 로 그려 달라고 세 번 고쳐 적었지만 생성기는 매번
   * **지평선이 가운데인 보통 눈높이 사진**을 그렸습니다. 그 사진을 50 m 방 벽에 통째로 붙이면 지평선이
   * 25 m 높이에 걸려 나무가 거인이 되고, 실측으로 세운 방 크기가 뜻을 잃습니다.
   *
   ...
   *
   * 실측(6×4×3 m 방): 생성기가 칸마다 너비를 조금씩 달리 그려 잘린 면의 비율이 앞 벽 1.9·뒤 벽 1.74·왼쪽 1.27
   ...
   *
   * 돔은 카메라가 크게 움직이지 않는 씬을 위한 것입니다.
   * 돔 반지름은 방 가로·깊이의 절반, 눈높이는 1.6 m(파노라마를 찍은 높이). 바닥 면(`faces.bottom`)이 걸려 있으면 그 실측 지도를
   * 가로×깊이 평면으로 깝니다 — 지면 투영 바닥은 카메라가 가운데서 멀어지면 번지기 때문입니다(헤드리스 실측).
   */
  panorama?: string;
  /**
   * **호리존 방** — 여섯 안쪽 면이 **한 가지 색**인 스튜디오(사이클로라마). 있으면 이 방은 호리존입니다.
   *
   * 제품 컷처럼 «배경이 없는 것이 배경» 인 구도를 위한 방입니다 — 색을 직접 고릅니다.
   *
...
export interface GlbTrack {
  ...
  url: string;
  /** 프로젝트 폴더에 정리한 파일. 다시 열 때는 이것으로 읽습니다. */
  filePath?: string;
...
export interface MotionTrack {
   ...
   *
   * 레이어에 «수화(춤선…)» 처럼 괄호로 띄우고, 모션을 갈면 이름도 같이 갑니다.
   *
...
export interface MotionKey {
   ...
   *
   * 천천히 걷다 빠르게 옮겨 가다 다시 천천히 — 곡선이 트랙에 하나뿐이면 그런 구간별
   * 리듬을 만들 수 없습니다.
   */
  easing?: EasingCurve;
  /**
 ...
 *
 * 편집 프로그램은 모두 «있는 구간» 을 막대로 채워 보여 줍니다. 카메라 구간과 같은 모양이라
 * 디자인도 통일되고, 누가 언제부터 언제까지 있는지가 한눈에 읽힙니다.
 *
 * # 키 여러 개가 아니라 막대 하나
 *
...
function layerFromLegacy(track: LegacyVisibilityTrack, id: string): TargetLayer | null {
  ...
  /*
    ...
    - 첫 키가 **0초의** «사라짐» → 처음엔 없다가 다음 «나타남» 부터(0초 사라짐 · 2초 나타남).
  */
  const first = keys[0];
 ...
 *
 * 가리는지 아닌지는 컷 내내 같지 않습니다 — 인물이 방 밖에 있는 동안은 벽이 가려야 하고,
 * 그가 들어오는 순간부터는 그 벽이 비쳐야 다가오는 장면을 담습니다.
 *
 ...
 *
 * 구도에서 뺀 영상은 **구도별로** 모입니다. 컷에는 «지금 쓰는 것» 한 편만 적히는데(`Cut.refVideoPath`), 실제로는 길이·나눠 뽑기·카메라를 바꿔 가며
...
export interface CompositionState {
   ...
   *
   * 편집 도구처럼 레이어를 쌓습니다 — 0~2초 달리 인, 2~3초 팬 좌 하는 식으로.
   ...
   *
   * 색이 그 덩어리의 이름표가 되어 프롬프트로 이어집니다 — «노란 구 덩어리는 안개» 처럼.
   */
   ...
   * 손맛이 달라 그 자동값이 여전히 빠르거나 느립니다 — 카메라가 휙휙 넘어가면 구도를 잡을 수 없습니다.
   ...
   *
   * 고를 것은 «방을 만들어 배경을 넣을까, 구도만 잡을까» 둘이면 충분합니다. 축척을 맞추려고
   * 손잡이를 자꾸 늘리다 보니 아무도 못 쓰는 화면이 됐습니다 — 파노라마·HDRI·배경 고정/함께·
   * 배경 눈높이·축척 맞추기·돔·3D 세트를 전부 걷어냈습니다.
   */
   ...
   *
   * 방을 세우면 밖에서 본 구도에서 안의 인물이 하나도 안 보여 구도를 잡을 수가 없습니다.
   * 걷는 경우는 «카메라 밖·인물 안·그 벽이 사이를 막을 때» 로 좁혀서, 반대쪽 «가리기»
   * (문 밖 인물을 방 안에서 볼 때)는 그대로 살아 있습니다.
   * 끄면 진짜 건물처럼 밖에서는 안이 안 보입니다.
   *
   * # 사람이 체크한 «가릴 면» 은 켜도 안 걷습니다
   *
   * 한때 안쪽 면까지 걷었더니, 가릴 면으로 체크해 둔 벽이 밖에서 보면 아무것도 안 가렸습니다.
   * 그래서 걷는 것은 **외벽 그림 때문에 자동으로 가리는 바깥 껍질**
   * 뿐입니다(`applyWallCutaway`).
   *
   * # 이름이 `wallCutaway` 가 아닌 까닭
   *
   * 그 사고를 고치며 잠깐 기본을 꺼짐으로 뒀다가 다시 켰습니다. 그 사이 앱이 정규화한
   * `wallCutaway: false` 를 **자동저장했을 수 있습니다** — 사람이 끈 것과 구별이 안 되는
   * 값이라, 같은 이름으로 기본만 바꾸면 이미 저장된 구도는 계속 꺼진 채로 남습니다.
   * 그래서 이름을 새로 달고 옛 값은 읽지 않습니다.
   */
...
// ── 호리존 방 ─────────────────────────────────────────────────────────────
/*
  제품 컷처럼 배경 없이 색만 두는 스튜디오 방입니다. 색의 모양·기본값·프리셋·문장은 **여기 한 벌**입니다 — 방 세우기(`addRoomIn`)·환경 탭·뷰포트·
...
export function normalizeComposition(
  ...
  */
  /*
    **새 구도에는 방이 없습니다.** 새 컷의 구도에 방이 미리 들어가 있으면, 쓸 일이 없는 방을 지우는 일부터 하게 됩니다.
  ...
  */
  ...
  return {
    ...
    */
    ...
    /*
      옛 트랙은 한 키에 자리·회전·크기가 함께 있었습니다. 속성별로 갈라
    ...
    */
    ...
    motionTracks: (saved.motionTracks || []).flatMap((track) => {
      ...
      /*
        트랙 «눈»(잠시 끄기)은 속성 줄에서 걷어 레이어 줄로 옮겼습니다 — 끄고 켜는 것은
        속성이 아니라 그 레이어이기 때문입니다.
        속성 줄에 켤 단추가 없어졌으니 꺼 둔 채 저장된 트랙은 **켜서** 읽습니다 — 영영 못 켜는 트랙이
 ...
 * # 왜 한 벌로 모았나
 *
 * 점검에서 드러난 것 — 같은 계산이 `summarizeCompositionCamera` 와 `cutPrompt.ts` 에 **두 벌**
...
export interface CompositionCameraSummary {
  ...
  subjects: {
     ...
     *
     * Seedance 2.5 는 `THE WOMAN 화면 왼쪽, x 42%, y 44%` 처럼 **숫자로** 자리를 받아 줍니다. 말로 「왼쪽에」 라고만 하면 컷마다 조금씩
     * 옮겨 가는데 백분율은 안 흔들립니다.
     *
 ...
 * 섭니다. 호리존은 사람이 **일부러** 세운 것(제품 컷용)이라 호리존이 이깁니다 — 장소 줄과
