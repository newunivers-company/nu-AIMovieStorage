 ...
 *
 * 일괄 생성과 한 번에 뽑기는 몇 분에서 몇 시간이 걸립니다. 그 사이 사람이 다른 작품을 보고 있어도
 * 답과 뽑은 그림·영상이 제 카드에 제 손으로 들어가야 합니다.
 *
...
interface Target {
   ...
   *
   * 일괄 생성을 돌렸는데 결과가 한 장도 안 붙고, 작업 서랍에 「그 인물 카드를 찾지 못했습니다」
   * 가 스물네 개 찍힌 적이 있습니다.
   *
 ...
 * 등록 해제 자체로는 «버려졌다» 와 «열쇠만 바뀌었다»(첫 저장으로 id 가 생겨
 ...
 * 보였습니다. 말하는 곳은 **부른 쪽 하나**이고, 여기는 까닭만 건넵니다.
 */
 ...
 * 쓰는 길은 저장이 거절돼도(두 창이 같은 작품을 열었을 때의 자물쇠) 그것을 모른 채 `after` 를
 * 부르고 참을 돌려줬습니다 — 일괄 생성은 «붙였다» 로 끝났는데 파일에는 없고, 존재하지 않는 id 로
 * 뽑기가 줄에 섰습니다. 그래서 지금은 **저장 결말을 본 뒤** 돌려줍니다.
 *
