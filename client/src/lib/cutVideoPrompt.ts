 ...
 * 컷 하나를 **영상으로** 뽑는 프롬프트.
 *
 * 컷마다 있는 프롬프트는 **그림**을 뽑는 것입니다. 한 컷이 그대로 영상이 될 수도 있어서,
 * 구도잡기 참조 영상과 인물·배경을 그대로 쓰되 영상용으로 따로 짓는 프롬프트가 필요합니다.
 *
 * # 그림 프롬프트와 무엇이 다른가
 *
...
export interface CutVideoPromptInput {
   ...
   *
   * 켠 토글이 **영상 프롬프트에는 아예 안 실리던 적**이 있습니다 — 키 이미지에만 붙고
   * 영상은 그냥 지나갔습니다.
   */
  lookEn?: string;
  /**
   ...
   *
   * 모델마다 프롬프트 문법이 달라, 보낼 모델을 알면 그 모델 규칙대로 짓습니다.
 ...
 *
 * 영상 프롬프트에 `natural visible pores, …, fine film grain` 이 **두 번** 박히던 일이 있었습니다.
 * 일괄 생성이 컷의 `styleTags` 에 자동 질감 칩을 **이미 합쳐 넣고** 그 위에
...
export function buildCutVideoPrompt(
  ...
  */
  ...
  /*
    **호리존이 장소를 이깁니다** — 컷 프롬프트(`cutPrompt.ts`)와 같은 규칙 한 벌(`horizonOverridesLocation`).
    ...
    긴 쪽(장소)을 골라 제품 컷이 카페 안에 섭니다(그림 쪽은 고쳐 두고 영상 쪽만 남아 있었습니다).
  ...
  */
  ...
  // 호리존 방이면 배경은 «단색 스튜디오» 라는 사실 — 컷 프롬프트와 같은 문장(`describeHorizonRoom`).
  ...
  /*
    ...
    비·연기 같은 효과보다 중요합니다 — 대사와 연기 지시가 프롬프트에 실려야 하는 까닭입니다.
  */
  if (input.acting?.trim()) {
    /*
      **명령형만 조용히 바로잡습니다.**

      번역기 없이도 말이 되어야 합니다. 답은 두 겹입니다 —
  ...
  */
  ...
  /*
    **금지는 «네거티브 칸이 있을 때만» 부정문으로 적습니다.**
    ...
    뽑은 영상에 쓸데없는 자막이 박히던 일이 있었습니다. `no text, no subtitles` 를 본문에 적으면
  ...
  */
  ...
  /*
    ── 형식과 잠금은 **맨 뒤** ──────────────────────────────────────────
    공개 프롬프트 8,961개를 센 자료(vflow)를 보고 넣었습니다.
 ...
 *
 * 컷을 그림으로만 뽑는 경우에도 그 그림이 컷의 대표로 등록되어야, 스토리보드가 그림으로
 * 짜입니다.
 *
