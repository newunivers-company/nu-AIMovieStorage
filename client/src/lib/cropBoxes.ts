 ...
 *
 * `SheetPanelCropper.tsx` 에서 떼어 냈습니다. 그리기 면과 목록 판이
...
export interface CropperSavedFile {
   ...
   * 기대 등록했습니다. 그래서 파일은 폴더에 멀쩡히 있는데 화면에서는 깨진 그림으로
   * 떴습니다. 지운 판은 이제 제 이름(`"erase"`)으로 갑니다.
   *
   * `"upscale"` 은 편집하지 않고 원본만 키운 새 파일(`…_업스케일_NNN`)입니다 — 원본 옆에 남습니다.
   */
  kind: "crop" | "mark" | "panorama" | "erase" | "upscale";
  /**
