 ...
 *
 * 컷 프롬프트는 써 두었는데 그림이 없어 아무 데도 안 이어진 때가 있습니다. 그림을 넣거나
 * 참고할 그림을 바꿨을 때, 이미 써 둔 프롬프트에서 **연결 부만** 갈아 끼우는 자리입니다.
 *
 * # 왜 LLM 을 다시 부르지 않는가
 *
...
export interface PromptLinkPerson {
   ...
   *
   * 한 번에 상세를 붓는 길로 지은 프롬프트라도 연결 자리는 미리 서 있어야, 나중에 그림을
   * 뽑는 순간 그 그림이 바로 걸립니다.
   */
  look?: string;
  /**
...
/*
  ── 그림이 아직 없는 것들의 «자리» ────────────────────────────────────
  아직 안 뽑은 그림도 프롬프트에 자리는 있어야 합니다 — 그래야 뽑는 순간 그 자리에 걸립니다.
...
function pendingLine(input: PromptLinkInput, lang: "ko" | "en"): string {
  ...
  /*
    ...
    한글 문단이 박힙니다 — 영문 프롬프트인데 한글이 섞여 나온 적이 있습니다. 영문 본문은
...
/**
 * **옛 판이 남긴 여러 줄 꼬리의 나머지.** 예전 `pendingLine` 은 생김새를 여러 줄 그대로 넣어,
 ...
 * 이 「@서진우_001 @서진우_001」 로 칩이 두 번 섭니다(마그니픽에 붙여 넣은 페이로드에서 확인). 꼬리 줄은 이미 태그가 서 있는 줄이라 손대지 않습니다.
 */
...
function relinkBody(text: string, input: PromptLinkInput): string {
  ...
  */
  ...
  /*
    ...
    로 시작하지 않아 주인을 못 찾았습니다 — 한 번 이었다가 그림을 뺀 뒤 다시 이어도 그 사람의
    태그가 본문에 그대로 남았습니다.
...
export function relinkPromptText(
  ...
  */
  ...
  /*
    ...
    ③ 이 나중에 붙은 걸음입니다. 손으로 레퍼런스 그림을 걸어도 프롬프트는 그대로였습니다 —
    그림이 없던 시절에 쓴 프롬프트에는 `@토큰` 이 **하나도 없어서** ② 가 바꿀 것을 못 찾았고,
    본문에는 「서진우가」 라고 맨 이름만 있었기 때문입니다. 그래서 이름 자체를 태그로 올립니다 —
    그림이 생기는 순간 그 그림이 걸리게 하는 길입니다.
  */
