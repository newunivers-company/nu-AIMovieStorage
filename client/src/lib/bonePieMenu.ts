 ...
 *
 * 한 켜에 서넛만 둡니다 — 상체·왼쪽·오른쪽 → 팔·손·다리 → 팔·팔꿈치·손·손가락 → 손가락 다섯 →
 * 그 관절. 관절 예순 개를 한 판에 늘어놓으면 겨눌 수가 없고, 몸을 따라 내려가야 자세를
 * 위에서 아래로 차례대로 맞춥니다.
 *
 * # 왜 데이터로 두는가
 *
 ...
 *
 * 이 켜에는 **팔·다리만** 둡니다. «손» 을 나란히 두면 손가락으로 가는 길이 팔 밑에 하나,
 * 손 밑에 하나로 둘이 되어 «어느 쪽으로 들어왔더라» 가 생깁니다. 손은 팔의 끝이니 팔 안에만
 * 둡니다.
 */
