 ...
 *
 * 씬은 컷 여럿을 묶은 것이고, 스토리보드는 **씬마다** 한 장입니다. 그 한 장을 마그니픽에
 * 올려 바로 씬 영상을 뽑으므로, 시트와 함께 쓸 프롬프트까지 여기서 만듭니다.
 *
 * # 크기는 **내용이 정합니다**
 *
 * 칸이 하나뿐인 씬도 6000×6000 을 굽고 있었습니다 — 한 칸이 5,700px 까지
 ...
 * **칸은 작품 비율을 따릅니다.**
 *
 * 점검에서 잡힌 것입니다. 칸이 16:9 로 못 박혀 있어서, 숏츠 작품(9:16)의
 ...
 *
 * 키 이미지와 컷 정보를 한 장에 합쳐, 인물 시트와 같은 규격의 스토리보드를 만듭니다.
 ...
 *
 * 칸에 들어가는 것은 그 컷의 키 이미지이고, 키 이미지가 없으면 구도 그림이 대신 섭니다.
 *
...
function guideAsCell(cut: Cut): GeneratedImageAsset | null {
  /*
    ...
    구도 그림이든 생성 결과든 **보이기만 하면** 단추가 살아야 합니다 — 경로만 보던 탓에,
    화면에는 구도가 보이는데 «스토리보드 만들기» 는 꺼져 있었습니다.
  */
...
export function buildStoryboardVideoPrompt(input: {
   ...
   * 시트 자체의 `@파일이름`.
   *
   * 마그니픽은 올린 그림을 **파일 이름**으로 부릅니다 — «첨부한 스토리보드» 라고 적으면 어느 그림인지
   ...
   * 영상 화면비(`"9:16"` …).
   *
   * 점검에서 드러났습니다 — 씬 영상에는 화면비가 **아예 안 실리고**
  ...
  /*
    러닝타임은 **문장에 적지 않습니다.**
    길이는 생성기의 설정 칸이 정하고(`composeInMagnific` 이 초를 따로 넘깁니다), 프롬프트에
  ...
  */
  ...
  /*
    ── 무엇을 무엇으로 그릴까 ──────────────────────────────────────────
    인물·배경·소품을 무엇으로 바꿔 그릴지, 그리고 효과(VFX)까지 글로 실어야 합니다.
  ...
  */
  ...
  /*
    ...
    들어가면 생성기가 그 글자를 그립니다. 본문만 씁니다. 가르는 규칙은 `splitLinkTail` 한 벌.
  */
  ...
  cells.forEach((cell, index) => {
    ...
    /*
      ...
      글만 적으면 «화면의 어디에서» 가 빠집니다. 연기 지시·카메라 무빙·인물 이동은 그림에 표시하고
      글로도 실으며, 그려진 번호와 같은 번호로 짝을 지어 적습니다.
    */
  ...
  */
  ...
  /*
    ...
    러닝타임을 문장에 안 적던 까닭은 **길이를 몰랐기 때문**입니다. 지금은 컷들을 더해 정확히 압니다. 화면비도 프로젝트가
