 ...
 * 구도의 «색 → 그게 무엇인가» 대조표.
 *
 * 구도 캡처는 **이름표 없이** 찍힙니다 — 이름표를 남기면 생성기가 그것을 그림 속 글자로
 * 그려 버려서 컷마다 다른 낙서가 박힙니다.
 *
 * 그런데 이름을 지우면 저 회색 마네킹이 누구인지 알 길이 사라집니다. 그래서 화면에서
 * 사람과 소품을 구분하던 **식별 색**(`characterColor`)을 그대로 글로 옮겨 프롬프트에
 * 싣습니다 — «파란 사람은 @냥이_시트_001, 빨간 상자는 소파». 색이 그림과 글을 잇는
 * 고리라, 소품 색도 구도잡기에서 바꿀 수 있어야 이 대조표가 뜻을 갖습니다.
 */
...
export function characterLegend(
  ...
  // «저 파란 사람» 이 통합니다. 여기서 따로 세다가 셋이 어긋난 적이 있습니다.
 ...
 *
 * 별도 줄에 «분홍 인물은 여울입니다 — 시트는 @…» 라고 적어 주는 것보다,
 * 본문에서 그 사람을 부르는 자리마다 칩이 서 있는 편이
 ...
 * ## 별칭도 같은 태그로
 *
 * 영문 본문은 인물을 로마자(«Seo Jinwoo (Nishimura Jin)»)로 부릅니다.
 ...
 * 길어졌습니다. 경계 검사로는 못 막는 모양이라 살아 있는 태그 구간을
...
export function characterAliases(
  ...
  // 가운뎃점은 자판마다 다른 글자로 들어옵니다 — U+00B7(·)·U+318D(ㆍ, 천지인)·U+30FB(・). 하나만 보면 두 이름이 한 별칭으로 붙습니다.
