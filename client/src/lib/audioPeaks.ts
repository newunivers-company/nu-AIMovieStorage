 ...
 *
 * 여태 노래 줄에는 «구간» 칸만 있었고, 구간을 안 나눠 두면 **텅 비어** 있었습니다 —
 * 노래를 올렸는지조차 줄만 봐서는 알 수 없었습니다. 영상 편집 도구들이 레이어 막대 위에
 * 파형을 얹는 것과 같은 까닭입니다. 파형은 「소리가 여기 있다」 를 한눈에 말해 주고,
  ...
  try {
    ...
    for (let bucket = 0; bucket < PEAK_BUCKETS; bucket += 1) {
      ...
      /*
        ...
        그때마다 배열을 새로 만듭니다 — 이 자리만 두는 예외입니다.
      */
