...
/*
  ── 저장해 둔 카메라 구도 ─────────────────────────────────────────────────
  잡아 둔 구도를 저장해 두고, 카메라를 더해 다른 구도를 또 잡습니다. 지우기도, 켜 둔
  카메라를 옮겨 그 자리에 다시 저장하기도 됩니다.
*/
 ...
 *
 * 켜 둔 카메라를 옮겨 그 자리에 다시 저장합니다. 이름은 그대로 둡니다 —
 ...
 *
 * 0~2초 달리 인, 2~3초 팬 좌처럼 **이어 붙이는** 것이 보통입니다. 새 클립을
...
export function addCameraMoveIn(
  ...
  */
  ...
  /*
    ── 자유 클립의 **시작점** ────────────────────────────────────────────
    자유 클립의 시작점은 **그 시각의 카메라 구도**입니다 — 앞 클립이 데려다 놓은 자리에서
    이어 받아, 거기서부터 키로 바꿉니다.
 ...
 *
 * 자유 경로의 시작은 **그 타임라인 자리의 카메라 값**이어야 합니다.
 *
 ...
 *
 * 정해진 무빙 프리셋으로는 안 되는 길이 있어 **자유롭게 그리는** 갈래를 둡니다.
 ...
 *
 * 키마다 자리·회전·줌을 손으로도 적을 수 있어야 합니다. 화면에서 카메라를 옮겨
 ...
 *
 * 키가 하나뿐이면 **그 값 그대로**입니다. 첫 키를 찍은 뒤로는 값을 고칠 때마다 키가
 * 저절로 서야 합니다 — 트랙이 생기는 순간부터 그
 ...
 * ── 자세 줄을 **관절별로 펼치기** ───────────────────────────────────────────
 *
 * 자세 줄에 키만 찍혀 있으면 **어느 관절을 움직인 것인지** 알 수가 없어 고칠 수도 없습니다.
 *
 ...
 *
 * 오른손만 고쳤는데 오른팔에도 키가 찍히던 일이 있었습니다. 자동 키가 «찍은 관절» 을
 ...
 *
 * 접힌 줄에는 아래 관절들의 키 자리가 그대로 뜨고, 그 점을 옮기면 같은 자리의 키가 함께
 * 갑니다(애프터이펙트의 요약 키와 같은 셈).
 *
 ...
 *
 * 완급 곡선 하나가 모든 키에 걸리면 구간마다 다른 속도를 줄 수 없습니다.
 */
 ...
 *
 * 카메라뿐 아니라 인물·소품의 이동·회전도 같은 키로 다룹니다.
 *
...
export function motionValueOf(
  ...
  /*
    **GLB 도 같은 트랙으로 움직입니다** — 이동·회전·크기를 타임라인에서 그대로 다룹니다.
    크기는 한 값(균등)이라 세 축에 같은 값을 넣습니다 — 트랙은 벡터로 적고, 화면은 x 만 씁니다.
  */
...
export function shiftMotionKeysIn(
  ...
  const tracks = motionTracksOf(current);
  /*
    **대상·채널을 가리지 않습니다.** 줄별로만 잡히면 한 동작을 통째로 옮길 수가 없습니다.
    한 사람의 이동·회전·자세는 **같은 순간의 한 동작**이라, 채널마다 따로 옮기면 팔만 먼저 가는 춤이 됩니다.
  */
 ...
 *
 * 프리셋 이름이 «오빗 90도» 라고 해서 반드시 0~90도를 돌려야 하는 건 아닙니다.
 *
...
export function addAmountKeyIn(
  ...
  /*
    **첫 키를 찍을 때는 양 끝을 함께 세웁니다** — 0초에 0, 끝에 프리셋 이동량.
    ...
    — 값을 고쳐도 아무 일이 안 일어나는 것처럼 보였습니다.

    양 끝을 세워 두면 손대기 전과 **똑같이 움직이면서**, 시작·끝을 각각 고칠 수 있습니다.
    「시작은 몇 도, 끝은 몇 도」 가 그대로 됩니다.
  */
 ...
 *
 * 관절만 따라가서는 안 되고 몸통의 이동·회전도 따라가야 합니다. 우리 캐릭터는 관절에 회전값만 있고 몸 전체에
 * 이동·회전이 따로 있어서, 한 순간이 세 트랙(position·rotation·pose)의 키 세 개가 됩니다.
 *
 ...
 *
 * 위쪽 무빙 목록에서 끌어다 떨구면 그 줄의 무빙이 갈립니다.
 *
 ...
 *
 * 줄은 **시각 순**으로 그려지므로 «위» 는 곧 «먼저» 입니다. 그래서 차례를
