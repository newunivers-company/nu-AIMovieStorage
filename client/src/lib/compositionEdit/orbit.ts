...
/*
  ...
  되돌리지 않으려고요. 기본값으로는 조금만 끌어도 화면이 획획 돌아 자리를 맞출 수가
  없었습니다.
 ...
 * - 거리는 **네제곱근**입니다. «가까이서는 천천히, 멀리서는 빠르게» 가 손맛에는 맞지만
 * 1제곱으로 하면 100m 에서 25배가 되어 못 씁니다 — 회전은 결국 각도라
 ...
 *
 * 1초의 자세를 3초까지 붙들어 두고 4초부터 움직이고 싶을 때, 3초에는 값이 달라진 게 없어
 * 자동 키가 안 찍힙니다. 그 «값이 같은 자리» 에 키를 박는 길이 필요합니다.
 *
