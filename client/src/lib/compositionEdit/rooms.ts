...
/*
  ...
  인물이 뜹니다 — 화면에서는 사람이 하늘에 떠 있는 꼴로 보입니다.
  ...
  «방» 은 배경 여섯 면을 **밑면이 y=0 인 유한 큐브**에 붙입니다.
    ...
    · 방을 줄이면 1.7m 인물이 방에서 차지하는 비율이 커져 배경보다 커 보입니다.
      이것이 «큐브 크기로 배경을 키우고 줄이는» 동작의 실체입니다.
...
export function roomsOf(current: CompositionState): CompositionRoom[] {
  /*
    **없으면 없는 대로** 돌려줍니다 — 방은 «방 추가» 로 사람이 세우는 것이지 처음부터 있는 것이 아닙니다.
 ...
 * 흔해서(실내라고 층고가 높다는 보장이 없습니다) 정육면체로는
 ...
 *
 * 실외도 돔과 상자 둘 중에 고릅니다. 상자로 바꾸면 돔 텍스처를 풉니다 —
 ...
 *
 * 방을 세울 때 실내·실외를 고릅니다 — 이미지 생성이 전개도를 뽑을지 파노라마를 뽑을지가 여기서 갈립니다.
 * 실외는 한 변 100 m 돔이 기본입니다(50 m 로는 하늘이 코앞에 섰습니다).
 *
 * 돔 반지름이 곧 «둘레까지의 거리» 라, 실내 치수를 물려받으면 나무가 사람만 해집니다. 기본 100 m 는 그저 출발점이고
 * 가로·깊이·층고 칸에서 언제든 바꿉니다.
 */
/**
 * 실외 방(돔)의 **기본 한 변**(m). 출발점일 뿐이고 칸에서 언제든 바꿉니다.
...
/** 방의 세 갈래 — 실내(6면 상자) · 실외(돔/큰 상자) · 호리존(단색 스튜디오). */
...
export function addRoomIn(
  ...
  */
  // 실외는 «세상» 이라 언제나 원점입니다(그 안에 실내 방들이 섭니다).
...
export function setRoomPlacementIn(
  ...
  const moved = patchRoomIn(current, roomId, (room) => {
    /*
      **실외는 늘 가운데**입니다. 실외는 돔이든 큰 상자든 «세상» 이라, 옆으로 밀면 그 안에
      있어야 할 방들이 밖으로 나가 버립니다.
 ...
 *
 * 방을 세우기 전에는 «구도만» 모드 그대로 — 넓은 바닥 격자 위에서 방을 더해 갑니다.
 *
 * 방을 기본으로 안 세우게 바꾼 뒤, 이 값이 «켜짐» 으로 남아 바닥 격자가 **방 크기**(=0)를 따라가 화면이 통째로
 ...
 *
 * 비율은 사람이 재지 않고 **그림이 정합니다** — 여섯 면을 걸면 방 모양이 저절로 따라옵니다.
 *
