 ...
 * 영상 → 사람별 관절 좌표(모션 캡처).
 *
 * 구도잡기에 영상을 올려 그 안 사람의 동작을 캐릭터에 옮깁니다. 영상에 사람이 셋이면 1번은 a 캐릭터, 2번은 b 캐릭터처럼
 * 번호로 짝을 짓습니다 — 댄스 커버 같은 영상으로 뮤직비디오를 엮으려고 낸 길입니다.
 *
 ...
 * 나왔습니다(실측에서 300 장 가운데 2명 이상이 나온 건 40 장). 모델이 화면을 224 픽셀로 줄여 보니 멀리 선 사람은 70 픽셀도 안
 ...
 * (19 초짜리 실측). 초록 원피스와 회색 조끼는 색이 확 달라 이것 하나로 대부분 갈립니다.
 ...
 *
 * 실측(케이팝 커버 댄스, 초당 30 장): 오른쪽 댄서 한 명이 10 초 동안 세 번호로 쪼개졌습니다. 장 사이 잇기는 1 초를
...
export async function captureVideo(
  ...
  /*
    ...
    첫 장만 되풀이해 검출하는 일이 있었습니다(모든 장의 좌표가 소수 셋째 자리까지 같았습니다).
  */
  ...
  const poseIn = (region: Region): FrameDetection | null => {
    ...
    /*
      ...
      **누운 자세**를 내놓은 일이 있었습니다. 몸통이 안 보이는 장은 키를 비워 두는 편이 앞뒤 키 사이를 잇는
      보간보다 훨씬 나쁩니다.
    */
  ...
  /*
    ...
    일이 많았습니다(같은 장이 1.4 배 자르기에선 실패, 1.8 배에선 성공). 처음 결과가 넉넉히 확실하면 더 보지 않아
    대부분의 장은 한 번으로 끝납니다.
  */
 ...
 * 보통 = 케이팝 커버 댄스 실측으로 고른 값(아래 `smoothSamples` 주석).
 */
 ...
 * 만큼 **빠른 동작의 끝이 뭉개졌습니다** — 케이팝 커버 댄스 실측(초당 30 장, 손목·발목 이동 속도 상위 2 %):
 *
 * | 방식 | 가만히 앉은 사람 | 춤추는 사람 |
