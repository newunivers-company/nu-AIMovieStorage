 ...
 *
 * 우리 캐릭터는 관절에 회전값만 있고 이동·회전은 몸 전체에만 있습니다. 관절만 따라가면 제자리에 선 채 팔다리만 흔들어서,
 * 이동과 몸통 회전도 같이 따라가야 합니다. 그래서 캐릭터를 IK 식으로 따로 만들지 않고, 한 순간을 세 갈래로 나눕니다.
 *
...
export function retargetPerson(
  ...
  options: {
     ...
     *
     * 쓸지 말지를 체크 하나로 가르고, 나중에 언제든 도로 드러낼 수 있게 갈라 둡니다.
