 ...
 * 모델**(앱이 직접 돌리는 것) 둘입니다.
 *
 * Suno 쪽은 API 로 프롬프트만 뽑아 밖에서 쓰고, 로컬 쪽은 앱 안에서 바로 뽑습니다.
 *
 * 로컬 쪽이 **ComfyUI 가 아닌** 까닭: 컴피UI 를 깔고 켜 두어야 하는 도구는 이 앱에서 쓰기에
 * 문턱이 너무 높습니다. 미니맥스의 **MiniMax-Music3** 가 오픈 웨이트로 나와서,
 * 컴피UI 를 거치지 않고 우리가 직접 돌립니다
...
export interface BgmTrack {
   ...
   *
   * 인물·장소 프롬프트와 같은 방식입니다 — 생성기에 넣는 것은 영문이고(태그 어휘가 영어),
   * 한글은 사람이 읽고 고치는 자리입니다. 두 칸이라야 골라서 복사할 수 있습니다.
   */
  styleKo?: string;
  styleEn?: string;
  lyricsKo?: string;
...
export type BgmToolId =
  | "suno"
  /** 로컬 — 어느 엔진인지까지 고릅니다. 둘은 받는 칸도 나오는 길이도 달라 «로컬» 한 갈래로 묶을 수 없습니다. */
  | "local-minimax"
  | "local-acestep"
...
/*
  ── 이 도구들이 실제로 받는 칸 ─────────────────────────────────────────
  음악은 프롬프트 한 덩어리를 짜 넣는 일이 아니라 **칸을 채우는** 일입니다. 그래서 그림·영상 카드의
  프롬프트·네거티브 짜임을 그대로 가져오면 실제로 쓰이지 않는 칸이 생깁니다. 셋이 무엇을 받는지
  **직접 확인했습니다**(Suno 는 V6 기준).
 ...
 *
 * v5 때 적어 둔 200자·3,000자를 V6 가 된 뒤에도 그대로 쓰고 있었습니다.
...
/*
  토글을 넉넉히 둡니다 — 적으면 결국 손으로 적게 되고, 손으로 적으면 생성기가 모르는 말이 섞입니다.
...
/*
  ...
  한 목록에 «instrumental» 을 끼워 두었더니 보컬의 «연주곡» 과 같은 말이 두 군데가 되었고,
  연주곡인데 verse·pre-chorus 를 고를 수 있었습니다.
...
export function createBgmTrack(): BgmTrack {
  return {
    ...
    instruments: [],
    tempo: "",
    // 길이는 **비워 둡니다.** 숫자를 미리 박아 두면 적지 않을 길이 없어집니다 —
    // 비면 생성기가 곡 구조에 맞춰 알아서 정합니다.
    durationSeconds: "",
    lyrics: "",
 ...
 *
 * 도구들이 받는 것이 프롬프트 한 덩어리가 아니라 칸 둘이라, 나오는 것도 **스타일 한 줄 + 가사 한 판**입니다.
 *
...
export function buildBgmStyle(track: BgmTrack): {
  ...
  /*
    ...
    실측에서 150초를 요청하고도 22초를 받았고, 구조 태그를 넣으니 75초가 나왔습니다.
  */
