 ...
 *
 * 구도잡기에서는 덩어리를 묶어 색을 입혀 두고, 프롬프트에서 «노란 덩어리는 안개로» 처럼
 * 그 색과 바꿔 그릴 것을 짝지어 줍니다. 앞에서 만든 캐릭터 시트·에셋 시트가 있으면
 * 그 짝을 **손으로 적지 않고** 여기서 자동으로 맺습니다.
 *
 * # 왜 «색» 으로 가리키는가
 *
 ...
 * 영어 복수형.
 *
 * 점검에서 드러났습니다 — `${kind.en}es` 로 만들고 있어서 `spherees` ·
...
export function describePeople(
   ...
   *
   * 캐릭터 특징 칸에 «영상 프롬프트에서 이 인물의 연기 기준이 됩니다» 라고 적어 두고도,
   * 그 값을 읽는 코드가 시트 굽기밖에 없었습니다. 적어 둔 것이 아무 데도 안 가는
   * 칸이었던 셈입니다.
   */
  ...
  /*
    ── 화면에서의 자리를 **숫자로** ──────────────────────────────────────
    Seedance 2.5 는 `THE WOMAN 화면 왼쪽, x 42%, y 44%` 처럼 숫자로 자리를 받아 줍니다.
    말로 «왼쪽에» 라고만 하면 **컷마다 조금씩 옮겨 갑니다** —
