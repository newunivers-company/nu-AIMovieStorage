...
export type LlmTask =
  ...
  | "cutPrompt"
  /**
   * 컷 **영상** 프롬프트. 상황·배경·환경·인물·구도를 아주 자세히 적어야 쓸 만한 영상이 나옵니다.
   *
  ...
  | "bgmPrompt"
  /**
   * 모캡 키 다듬기 — 뽑힌 키프레임 값을 LLM 이 읽고 다듬은 값을 돌려줍니다.
   * 흔들림 후보 구간의 숫자표를 읽고 «인식 오류인가, 의도한 빠른 동작인가» 를 가립니다(`motionCleanup.cleanupPrompt`).
   */
  | "motionCleanup"
  /**
   * **평소 말투 → 프롬프트 말.** 적은 그대로 보내면 생성기가 못 알아듣는 말이 섞입니다.
   *
...
export const REASONING_EFFORT_OPTIONS: {
  ...
  { id: "medium", label: "보통" },
  { id: "high", label: "깊이" },
  // 필요할 때만 올려 쓰는 단계입니다 — 요금이 크게 올라가므로 기본값으로 쓰지 마세요.
...
export const DEFAULT_TASK_MODELS: TaskModelMap = {
  ...
  */
  ...
  // ── 글쓰기 ──────────────────────────────────────────────────────────
  // 시트 프롬프트는 정형적인 요청이라 싼 단으로 충분합니다 — 비싼 단을 써도 결과가 달라지지 않았습니다.
...
export interface LlmCallOptions {
   ...
   *
   * OpenAI 는 배경 모드로 보내므로 앱을 껐다 켜도 서버가 답을 들고 있습니다. 서버에 그 답이 없으면(만료·잘못된 id) `isLlmResumeGone` 인 오류가 나고,
 ...
 * 통째로 버렸습니다 — 17쪽짜리 대본에서 «캐릭터·씬이 하나도 안 들어간» 원인이 이것이었습니다.
 *
...
export async function toLlmImage(
  ...
  /*
    ...
    (실제로 파일 경로를 그대로 넘겨 일괄 생성이 0초에 죽은 적이 있습니다).
 ...
 *
 * 요금은 바뀝니다. 모델을 더하거나 갈아 끼울 때 이 표도 같이 고쳐야 합니다.
 */
