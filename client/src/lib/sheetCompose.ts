 ...
 *
 * 규격은 손으로 정할 수 있습니다. 처음에는 배치 좌표를 «긴 변 = 6000» 기준으로 저장하고
 * 굽는 순간에 규격에 맞춰 통째로 줄였습니다. 그러자 4000 시트는 6000 시트를 축소한 같은
 * 그림이 되어 규격을 바꾸는 뜻이 없어졌습니다 — 그림은 뽑힌 크기 그대로 들어가야 합니다.
 * 그래서 이제 칸의 좌표·크기는 규격의 실제 px 이고, 그림을 넣으면
 ...
 * (그림은 뽑힌 크기 그대로 들어가야 합니다) — 시트보다 큰 칸만 비율을 지켜 시트에
 ...
 * 따로 만들면 프로필 칸이 빠집니다 — 실제로 이름 한 줄만 남은 시트가 나왔습니다.
 * 긴 글(성격·말투·버릇·배경·연출 메모)은 표가 아니라
 ...
 *
 * 그림 안 위쪽에 찍으면 그림을 가리고, 아래 바깥에 찍었더니 바로 밑 상자와 붙어 어느 칸의
 * 이름인지 헷갈렸습니다. 그래서 상자 위 바깥. 시트 맨 위라
 ...
 *
 * 스토리보드는 컷의 키 이미지와 그 컷 정보를 합쳐, 시트와 같은 6000×6000 규격으로 굽는 판입니다.
 *
 ...
 *
 * 줄글로 늘어놓으면 «5.0초 · 고정 / 수화안 / 대사·연기: …» 가 한 덩어리로 보여 무엇이
 * 무엇인지 가려지지 않습니다. 표로 나눠야 읽힙니다.
 *
