 ...
 * 사라진 프리셋 id → 지금 id.
 *
 * 이동량을 직접 적게 된 뒤로 «달리 인» 과 «달리 아웃» 은 부호 하나 차이일
 ...
 * 이 갈래가 **앵커(기준점)를 쓰는가.**
 *
 * 오빗·달리는 «무엇을 중심으로» 가 움직임의 전부라 앵커가 없으면 뜻이 안 서지만,
 * 팬·틸트는 제자리에서 고개만 돌리고 트럭·크레인은 시선을 유지한 채 미끄러집니다.
 * 앵커를 옮겨도 그림이 안 바뀌는데 앵커 단추가 똑같이 보이면 안 먹는 이유를 알 수 없습니다 —
 * 그래서 단추 색으로 갈라 둡니다.
 *
...
export const SHOT_PRESETS: ShotPreset[] = [
  ...
  // ── 고정 ──────────────────────────────────
  {
    ...
    duration: 4,
    /*
      무빙을 **안 넣는 것**이 곧 고정이라, 목록에 두면 아무것도 안 하는 클립을 고르게 됩니다.
...
export interface CameraMove {
   ...
   *
   * «오빗 90도» 라고 반드시 0~90도를 돌아야 할 까닭은 없습니다 — 시작과 끝 각을 키로 적습니다.
   *
   ...
   *
   * 지우는 것과 달리 값이 남아, 껐다 켜며 견줄 수 있습니다(애프터이펙트의 눈 아이콘과 같습니다).
   */
  muted?: boolean;
}
...
export interface CameraKeyframe {
   ...
   * 이 키에서 **다음 키까지**의 완급. 없으면 클립의 곡선을 씁니다.
   *
   * 곡선을 클립에 하나만 두면 «구간마다 다른 리듬» 을 만들 수가 없습니다 —
   * 천천히 가다 빠르게 붙고 다시 느려지는 무빙이 한 클립 안에서 안 나옵니다. 애프터이펙트도
...
export function createCameraMove(shotId = "dolly-in"): CameraMove {
  ...
  return {
    ...
    /*
      ...
      «앵커를 계속 바라보기» 를 한 값으로 두면 갈래 하나가 반드시 망가집니다.
  ...
  */
  ...
  if (kind === "dolly") {
    ...
    /*
      ...
      그 점을 보려고 카메라가 고개를 돌립니다 — 앵커를 안 보게 해 두었는데도 다가가며
      도는 것처럼 보이는 것이 이것입니다. 자리와
 ...
 * 속도 곡선을 **말로** 옮깁니다.
 *
 * 3D 미리보기는 곡선을 제대로 먹고 있었는데(`evaluateCameraMove`),
 * **프롬프트에는 곡선이 한 글자도 안 실려 있었습니다.**
...
export function describeCameraMove(move: CameraMove | null | undefined) {
  ...
  /*
    영어 문장은 **촬영 용어**로 씁니다. 여태는 `${preset.id} camera move` 였습니다 —
    «snap-push camera move» 같은 우리 내부 이름을 그대로 보내고 있었습니다.
    생성기가 못 알아듣고 제멋대로 흔듭니다.
  ...
  */
  ...
  /*
    **속도 곡선을 말로 실어 보냅니다.** 3D 미리보기는 곡선을 제대로 먹고 있었는데
    프롬프트에는 곡선이 한 글자도 안 실려 있었습니다 — 앱에서 «급가속 후 정지» 로
    맞춰도 밖에서 뽑으면 등속으로 나왔습니다.
  */
 ...
 *
 * «0~2초 달리 인, 2~3초 팬 좌» 처럼 클립을 이어 붙여 한 컷의 무빙을 짭니다.
 *
