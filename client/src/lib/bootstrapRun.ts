 ...
 * 건드리고, 저장 규칙을 고치다 절차 사이에 낀 patch 를 놓치는 일이 생겨 갈랐습니다.
 * 옮기기만 했고 동작은 그대로입니다.
 *
...
export interface BootstrapStartOptions {
   ...
   *
   * 상황·환경·동작·표정 묘사가 부실하면 LLM 을 쓰는 뜻이 없습니다.
   ...
   * 씨댄쓰 2.5 는 30초, 미니맥스 H3 는 15초 — 고른 모델의 상한에 맞춰 컷을 나눠야 합니다.
   */
  videoModel?: string;
}
 ...
 *
 * 지금 무엇이 차례를 기다리는지 보이고, 앱을 다시 켜도 하던 일이 이어져야 합니다.
 *
 ...
 * **몇 프로인지**도 함께 적습니다 — 한 시간 가까이 도는 일이라, 숫자가 없으면 멈춘 건지
 * 도는 중인지 구분이 안 됩니다.
 */
async function runBootstrap(
  ...
  /*
    ...
    자리에 떨어집니다(4단계에서 실제로 났고, 세 왕복도 같은 모양입니다).
  */
  ...
  */
  ...
  /*
    **경로가 아니라 주소**를 넘깁니다. 예전에는 일괄 생성이 0초에 「Failed to fetch」 로
    죽었습니다 — `D:\…\안경.png` 같은 파일 경로를 그대로 줬는데,
  ...
  */
  ...
  /*
    **이름표를 함께 보냅니다**(`ref_img_1` · `ref_mov_2`).
    ...
    파일을 여러 장 올리면 «캐릭터는 어떤 그림, 동작은 어떤 영상» 을 가리킬 말이 필요합니다.
    파일 이름은 한글·공백·확장자가 섞여 LLM 이 자주 놓칩니다.
            ...
            await withResumableLlm(taskId, "details", (llm) =>
              requestJsonFromLlm<unknown>({
                ...
                /*
                  ...
                  8192 도 17쪽짜리 대본에서는 모자랐습니다 — 캐릭터·씬 구성에 프롬프트까지
                  한 답에 채워야 합니다. 한국어는 같은 내용에 토큰이 더 듭니다.
                  그래도 잘리면 `parseJsonResponse` 가 받은 데까지 건져 냅니다.
                */
                // `project-details.md` 가 인물·장소마다 외형 **60~120 어절**을 요구합니다 — 인물 여섯에
    ...
    */
    ...
    /*
      **1단계 목록은 버리지 않습니다.**
      ...
      그것을 통째로 버리고 빈 목록을 부어서, 화면에는 «주제 설정» 만 채워졌습니다 — 손으로
      만드는 것과 다를 바가 없었습니다. 값은 이미 치른 답입니다.
