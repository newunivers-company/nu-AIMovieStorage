 ...
 * **다른 작품에서 카드를 끌어옵니다.**
 *
 * 시리즈물은 편이 달라도 인물이 같습니다. 그 인물의 에셋도, 사는 집도 매번 다시 나옵니다.
 * 저장 구조를 건드리는 대신 «끌어오기» 단추 하나로 다른 작품의 카드를 가져옵니다.
 *
 * # 구조를 바꾸지 않습니다
 *
 ...
 * # 갈래를 섞지 않습니다
 *
 * 캐릭터는 캐릭터끼리, 배경·방은 배경·방끼리 따로 끌어옵니다.
 * 인물 목록에서 장소가 보이면 안 됩니다 — 고르는 자리가 흐려집니다.
 *
 * # 그림은 골라서 가져옵니다
 *
 * 카드 하나에 그림이 수십 장일 수 있습니다. 전부 복사하면 폴더가 금세 불어나고 대개는
 * 몇 장만 쓰다 변형합니다 — 그래서 **고른 것만** 옮깁니다.
 */
...
export interface BorrowCandidate {
  ...
  /**
   * 이 카드의 **변형들**. 체크한 것만 따라옵니다 — 원본에서 구도만 바꾼 클로즈업 같은
   * 변형이 많아, 통째로 끌어오면 쓰지도 않을 것이 대부분입니다.
   *
...
export async function borrowCards(request: BorrowRequest): Promise<BorrowResult> {
  ...
  for (const pick of request.picks) {
    ...
    */
    ...
    /*
      ── 고른 변형만 따라옵니다 ──────────────────────────────────────────
      체크한 변형만 따라옵니다. 원본에서 구도만 바꾼 클로즈업 같은 것이 많아, 변형이
      수십 개인 카드를 통째로 복사하면 폴더가 금세 불어나고 대부분은 이 작품에서 안 씁니다.
      ...
      */
      ...
      /*
        **저쪽 작품의 파일을 가리키는 칸은 전부 비웁니다.**
        ...
        점검에서 찾았습니다 — `...source` 로 통째로 베끼면서 `assets`
 ...
 * **다른 작품에서 방을 끌어옵니다.**
 *
 * 구도잡기에서 방을 끌어오면 그 방에 걸린 6면 그림·파노라마도 따라와야 합니다. 다만
 * **걸린 것만** 입니다 — 새 작품에서는 새 작품 것만 건사해야 하고, 안 그러면 목록이
 * 금세 길어집니다.
 *
 * # 적용된 것만 옵니다
 *
