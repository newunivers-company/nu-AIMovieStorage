...
export interface GeneratedImageAsset {
   ...
   *
   * 이게 없으면 시트를 다시 열어 고칠 길이 없습니다 — 시트 제작에 다시 들어가 손대면
   * 고쳐지는 게 아니라 새 시트가 하나 더 생깁니다. 결과 그림에 판을 붙여 두면
   ...
   * 세웁니다. 50 미터로 뽑은 공간이 구도잡기에서 5.5×5.2×4.4 로 섰습니다 — 크기는 카드에 넣었는데 구도잡기는 몰랐습니다.
   */
  faceSetSize?: FaceSetSize;
  /**
   * 자동 6면 커팅이 **이 그림을 이미 봤다**는 표시.
   *
   * 전개도를 넣으면 여섯 면까지 저절로 나와야 합니다. 전개도가 들어오면
...
export interface ReferenceImage {
   ...
   * 삭제)의 예외입니다. 부모 레퍼런스를 빼려다 «파일도 삭제» 경고를 보는 일이 있었습니다.
   */
  sharedFile?: boolean;
 ...
 * (규격을 줄이면 그림도 같은 비율로 줄어 결과가 똑같았습니다. 그림은 **뽑힌 크기 그대로**
 * 들어가야 규격을 바꾸는 뜻이 생깁니다). 옛 값은 `SheetLayout.coords`
...
export interface SheetPlacement {
   ...
   *
   * 스토리보드의 컷 정보는 표로 끊어 넣습니다. 줄글로 늘어놓으면
   * «5.0초 · 고정 / 수화안 / 대사·연기: …»
   ...
   * 파일 이름이 칸 위에 붙어 그림을 가렸고 바꿀 수도 없었습니다 — «전신»·«표정» 처럼
   * 뜻이 보이게 다시 이름 붙일 수 있어야 합니다.
   */
  label?: string;
  /**
   * 이 칸의 그림 위에 **덧그릴 표시**(동선·구역·자리).
   *
   * 스토리보드에는 연기 지시·카메라 무빙·인물 이동을 그림 위에 표시하고 글로도 적습니다.
   ...
   *
   * 대표 그림과 컷 정보를 한데 모아 규격화된 스토리보드를 만듭니다. 캡션(`label`)은 한 줄짜리 «칸 이름»
   * 이라 컷 정보를 담기엔 좁습니다.
   */
  notes?: string[];
 ...
 * 골라 쓸 수 있습니다 — 다른 인물에서 시트 제작을 열어도 같은 배치도가 목록에 떠야
 * 같은 칸 구성을 인물 수만큼 다시 잡지 않습니다.
 *
...
export interface SheetLayout {
  ...
  /** 배치도마다 규격 — 손으로 정할 수 있습니다. 없으면 6000×6000 */
  size?: SheetSize;
  /** 칸 이름을 시트에 굽는가. 기본 true */
  captions?: boolean;
  /**
   * 좌표계 표시. `"px"` 면 칸 좌표·크기·글자가 규격의 실제 px.
   * **없으면 옛 «긴 변 6000 기준»** 배치도라 `layoutToPx` 로 바꿔야 합니다(옛 저장분).
   */
  coords?: "px";
}
 ...
 * (이게 없으면 다시 편집할 때 고쳐지는 게 아니라 새 시트가 하나 더 생깁니다).
...
> {
   ...
   *
   * 한 인물에 «냥이의 어린 시절»·«냥이의 노인 버전» 처럼 원본을 여럿 둬야 할 때가 있습니다
   * (보유 에셋이 이미 그런 모양입니다).
   ...
   * 올라가고 여기는 비워집니다.
   *
   * 배치도가 인물마다 갇혀 있어 다른 인물의 시트 제작에서는 고를 수 없었고,
   * 같은 칸 구성을 인물 수만큼 다시 잡아야 했습니다.
...
export interface SceneVideoAsset {
   ...
   *
   * 한 번에 원하는 영상이 나오는 일이 드물어 선반에 여러 편이 쌓입니다. 그림 선반의
   * «별» 과 같은 뜻이고, 한 선반에 하나뿐입니다.
   */
  isPrimary?: boolean;
}
...
export interface Cut {
   ...
   * 받아 둔 컷 프롬프트들.
   *
   * 받아 둔 프롬프트는 이력에 남아야 하고, **어떤 항목을 켜고** 지은 것인지까지 함께
   * 남아야 나중에 되짚을 수 있습니다.
   *
   ...
   *
   * 구도잡기에서 뽑은 영상이 있으면 그것을 레퍼런스로 걸고 곧장 영상을 뽑습니다.
   * 그 영상에는 길이가 있으므로 생성기의 러닝타임도 같은 값으로 채웁니다.
   *
   ...
   *
   * 구도 없이 인물만으로 키 이미지를 뽑는 일도 있어서, 구도는 넣고 뺄 수 있어야 합니다.
   * 그때마다 프롬프트와 그 안의 @태그가 함께 달라져야 합니다.
   *
   ...
   *
   * 인물을 갈아 끼울 때 어느 그림을 레퍼런스로 올릴지 인물마다 고를 수 있어야 합니다.
   ...
   *
   * 컷 프롬프트는 컷 «그림» 을 짓는 글입니다. 구도 레퍼런스 영상·인물·배경을 가지고
   * 컷 «영상» 을 바로 뽑는 글은 따로 있어야 합니다.
   *
   ...
   *
   * 인물 시트와 레퍼런스 영상으로 인물을 갈아 끼워 영상을 뽑을 때, 대사와 연기 지시도 함께 실려야 합니다.
   ...
   * 대사 없는 풍경 컷에 연기 지시를 실을 이유가 없습니다.
   */
  techniques?: string[];
}
...
export interface Scene {
   ...
   *
   * 스토리보드는 6000×6000 한 장으로 굽고, 그 시트를 생성기에 그대로 올려 영상을 뽑을
   * 수 있게 프롬프트까지 같이 짓습니다.
   *
 ...
 *
 * 지금 작업 중인 단계는 **반만 칠합니다** — 「다음」 을 눌렀으니 지나간 것이지만
 * 아직 손대고 있는 단계라, 다 칠하지도 비우지도 않고 절반으로 둡니다.
 */
/**
 * 저장돼 있던 «돌고 있음» 표시를 전부 끕니다.
 *
...
export function settleLoading(draft: ProjectDraft): ProjectDraft {
  ...
  // 보유 에셋(assets)이 없는 옛 프로젝트 파일을 열면 `assets.length` 에서 터졌습니다.
  ...
  /*
    **빠진 칸은 기본값으로 채웁니다.**
    ...
    `refImages` 같은 배열이 통째로 없을 수 있는데, 화면은 그 배열을 그냥 훑습니다 — 실제로
 ...
 * 진행 표시를 **네 단계**로 옮깁니다.
 *
 * 배경은 별도 탭일 까닭이 없습니다 — 씬에서 구도를 잡을 때 배경과 배경 에셋을 만들어 맞추는 일까지
 * 한자리에서 돕니다. 그래서 단계가 «주제 · 캐릭터 · 배경 · 씬 · 확인» 다섯에서
...
export interface ProjectDraft extends ProjectContext {
   ...
   *
   * 대표 그림이 없으면 보드 카드가 밋밋해 어느 작품인지 구별이 안 됩니다 — 손으로
   * 등록하든 저절로 채워지든, 둘 중 하나는 되어야 합니다.
   *
   ...
   *
   * 방은 다른 씬의 다른 구도에서 다시 쓰니 라이브러리로 둡니다 — 방에 키 오브젝트로 넣은 것과
   * 거기 걸린 에셋까지 함께 따라와야 합니다. 컷이 아니라 **프로젝트**가 들고 있어야 다른 씬의
   ...
   * 시트 배치도 — **프로젝트 공용.**
   *
   * 다른 인물에서 시트 제작을 열어도 같은 배치도를 고를 수 있어야 합니다.
   ...
   *
   * 마그니픽으로 뽑을지 로컬로 뽑을지를 **작품마다**, 그리고 이미지와 영상을 갈라
   * 정해 둡니다.
   *
   ...
   *
   * MCP 로 뽑으면 어느 모델로 이미지를 짓고 어느 모델로 영상을 지을지, 해상도는 얼마로
   * 할지를 우리가 정해 보내야 합니다.
   *
   ...
   * **화면 비율** — 이미지와 영상 따로.
   *
   * 이미지와 영상의 화면 비율을 따로 정합니다 — 숏츠는 세로로 뽑아야 합니다.
   *
   ...
   *
   * 한 번에 뽑을 때도 상황에 맞는 로라를 걸 수 있어야 합니다.
