 ...
 *
 * 구도잡기에서 방 치수를 실측으로 잡아 놓고 배경은 눈대중으로 뽑으면, 세운 방과 붙인 그림이
 * 어긋나 인물 크기가 맞지 않습니다. 그래서 그 치수를 그대로 생성기에 건넵니다.
 *
 * # 치수는 «글» 이 아니라 «틀» 로 줍니다 — 실험으로 고른 것
 *
 ...
 * 가로·깊이는 뽑힌 그림이 정했습니다.
 *
 * 지금은 카드에 적은 치수로 뽑은 세트면 **그 치수가 정답**입니다(`CompositionRoom.faceRatio`). MCP 실측에서
...
export function buildUnfoldTemplate(
   ...
   *
   * 나노 바나나 결과: 칸마다 다른 색(빨강·노랑·초록·보라)이 그림 둘레에 **색 테두리**로 남고, 방 벽마다 틀 색이
   ...
   *
   * MCP 실측: 같은 문장에 이 틀과 «밝은 위 절반과 조금 어두운 아래 절반이 만나는 줄이 지평선» 한 줄을 더하자,
  ...
  for (const [face, a, b, c, d] of boxes) {
    ...
    if (!context) continue;
    /*
      칸은 바탕(#808080)보다 **아주 조금만** 밝게(#929292, 차이 18). MCP 실측: 밝은 회색(#c2c2c2) 칸이면 생성기가 그림을
 ...
 * (칸을 색이 아니라 자리로 부르는 문장). 옛 `buildUnfoldPrompt`(색 칸 문장)는 부르는 곳이 없어 남겨만 둡니다.
 *
 ...
 *
 * 카드에 50 m 라고 적어도 생성기가 그 넓이를 안 그리면 세팅한 뜻이 없습니다 — 실제 50 m 범위의 공간이
 * 여섯 면으로 나와야 합니다.
 *
 ...
 *
 * ## 문장은 마그니픽 MCP 로 나노 바나나 프로를 직접 돌려 고른 것입니다(판마다 2~4 장)
...
export function buildOutdoorAreaEnglish(side: number, place: string): string {
  ...
  /*
    카메라 문단을 장소보다 **앞**에 둡니다(앞쪽 글을 더 무겁게 따릅니다 — 실측으로 확인).

    «아래 절반은 땅만, 경계는 지평선 위로»: 이 두 줄이 없을 때는 생성기가 보통 풍경 사진처럼 숲을 화면 가운데에
    크게 채워 **숲 밑동이 지평선보다 칸 높이의 20~30 % 아래**(= 3~5 m 앞)였고, 25 m 벽에 붙이자 사람 옆의 나무가
    몇 배로 커 보였습니다. 틀의 어두운 아래 절반·밝은 위 절반을 말로 짚자 네 장 모두 밑동이 지평선 근처
 ...
 *
 * 실외는 벽이 없으니 여섯 면으로 접을 것이 없습니다 — 구에 감는 파노라마 한 장이 맞고, 육면체는 실내 몫입니다.
 ...
 *
 * 마그니픽 MCP · 나노 바나나 프로 실측(6×4×3 m, 판마다 2~3 장):
