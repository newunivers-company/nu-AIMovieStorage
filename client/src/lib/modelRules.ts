 ...
 * **모델별·버전별 프롬프트 규칙표.**
 *
 * 모델마다 프롬프트를 쓰는 규칙이 다릅니다. 공통 규칙과 모델별·버전별 규칙을 갈라 두어야
 * 고른 모델에 맞는 프롬프트가 나옵니다 — 화면에서 조립할 때도, API 로 보낼 때도 같습니다.
 *
 * # 왜 문서가 아니라 표인가
 *
 ...
 * # 버전을 왜 나누는가
 *
 * 실제로 저지른 잘못입니다 — Kling **2.6** 공식 문서의 제약(«중국어·영어만 소리로 낸다»)을
 * **3.0** 문서에 그대로 옮겨 적었습니다. 3.0 은 한국어를 잘 말하고, 다만 연기가 과장될
 * 뿐이었습니다. 같은 이름의 모델이라도 **판이 다르면 다른 모델**입니다.
 * 한 칸에 뭉뚱그리면 이런 사고가 납니다.
 *
 * # 공통 규칙은 여기 없습니다
 *
  ...
  // ── ByteDance Seedance ────────────────────────────────────────────────
  {
    ...
    /*
      **30초입니다.** 규칙표에 15 로 적혀 있었는데 같은 앱의 모델 문서는 30 이라고
      말하고 있었습니다. 2.5 는 «짧은 클립» 모델이 아니라 30초짜리
 ...
 *
 * 우리가 노리는 것은 실제 영화와 구분되지 않는 그림입니다.
 *
...
export function matchModelRule(text: string | null | undefined): ModelRule | null {
  ...
  /*
    **판 번호의 점과 하이픈을 같게 봅니다.**
    ...
    실측에서 `seedance-2-5-pro`(마그니픽 슬러그)가 **Seedance 2.0 으로**
 ...
 *
 * 영문 칸과 한글 칸이 서로 맞는지 보려면 먼저 섞인 것부터 찾아야 합니다.
 *
 ...
 *
 * 컷에서도 씬에서도 **모델을 먼저 고를 수 있어야** 그 모델에 맞는 프롬프트를 받습니다.
 * 그림도 영상도 같고, «구성» 을 누르면 여기서 정한 모델이 마그니픽에서도 골라집니다.
 *
 * # 왜 이 표가 필요한가
 *
 ...
 * 로 쓰면 아무도 안 골라서 — 화면 실측입니다 — 컷 예순두 개의 영상 프롬프트가
 ...
 *
 * 30초까지 뽑는 모델을 골랐으면 씬도 30초 기준으로 짜여야 합니다. 여태 일괄 생성 2단계는 모델을 모른 채
 * «1~10초» 로 못 박혀 있었습니다 — 규칙 표에는 30초가 적혀 있는데 요청에 실리지 않았습니다.
 * 모델을 모르면 null — 요청 문구가 10초로 봅니다.
 */
