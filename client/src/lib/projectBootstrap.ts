...
export interface BootstrapCut {
   ...
   * 이 컷이 몇 초짜리인가.
   *
   * 컷 하나가 그대로 영상이 되기도 합니다. 그때 러닝타임이 필요한데, 구도잡기를 열기
   * 전에는 잴 것이 없습니다. 기획 단계에서 받아
 ...
 *
 * 주제 한 줄에서 씬·컷·인물·구도·키컷까지 한 번에 세우는 흐름의 한 걸음입니다 —
 * 카드만 세우는 것이 아니라 **생성기로 보낼 프롬프트까지** 채워 놓아야 뜻이 있습니다.
 *
...
function buildCharacter(
  ...
  */
  ...
  return {
    ...
    /*
      ── 기록에도 남깁니다 ────────────────────────────────────────────
      규칙으로 지은 프롬프트도 이력에 남아야 나중에 되돌릴 수 있고, **어떤 칸이 켜져**
      나온 글인지까지 알아야 고쳐 쓸 데를 짚습니다.
...
function buildCuts(
  ...
  return scene.cuts.map((cut, index) => {
    ...
    */
    /*
      ── «질감 · 실사» 는 앱이 켭니다 ──────────────────────────────────
      일괄 생성이 만든 컷은 촬영·조명·색감·질감 네 줄이 통째로 비어 있었습니다. 앞 셋은 **판단**이라 LLM 이 고르게 했고(템플릿), 질감은 **규칙**이라
        ...
        ? buildCutVideoPrompt({
            ...
            /*
              **연출·질감을 프롬프트에도 싣습니다.** 예전에는 `styleTags` 를 컷에 켜 두기만
              ...
              생성기로 가는 글에는 그 말이 하나도 없었습니다 — 칸은 켜졌는데 글은 그대로인 꼴입니다.
            */
 ...
 * 통째로 버리고 빈 목록을 부었습니다 — 주제 칸만 채워지고 나머지는 손으로 적는 것과
 * 다를 바가 없었습니다. **값은 이미 치른 답**이라 버릴 이유가 없습니다.
 *
