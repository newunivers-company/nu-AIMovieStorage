 ...
 * # 왜 목록이 아니라 폴더를 믿는가
 *
 * 여태는 어딘가에 받아 둔 파일의 **경로만** 기억했습니다. 그러면 파일을 옮기거나
 * 지웠을 때 목록은 그대로 남아, 뽑기를 눌러 봐야 «없는 파일» 로 엎어집니다. 이제 엔진 폴더를
 ...
 * 다시 그립니다. 저장만 하고 알리지 않으면 세기·기본·이름 칸이 «쳐도 제자리» 가 되어
 * 값이 아예 안 바뀌는 것처럼 보입니다 — React 는 통제값(value)이 그대로면 입력을 그 값으로
 * 되돌립니다.
 */
 ...
 *
 * `bases` 는 Civitai 의 밑모델 갈래 이름(정확히 그 글자라야 걸립니다 — 모르는 이름이면
 * 0개). 갈래가 없는 엔진은 `keywords` 로 밑모델·이름에 든 낱말만 봅니다.
 *
 * 예전 표는 세 엔진만 있어서 나머지는 필터 없이 나갔습니다 — 한 엔진 것만 찾는데 다른
 * 엔진 로라가 섞여 쏟아졌습니다(Pony 로라가 그랬습니다). 이제 **엔진마다 반드시** 한 줄이 있습니다 — 새 엔진을 더하면 타입이 잡습니다.
 */
/*
  갈래 이름은 **짐작하지 않습니다.** «MiniMax»·«Hailuo» 로 찔러 0개가 나와 «갈래가 없다» 고
  적었는데, Civitai 에는 71개가 있었습니다 — 진짜 이름은 «MiniMax H3»(띄어쓰기) 였습니다.
  그래서 아래 이름은 전부 **검색 결과의 판(version)이 실제로 달고 있던 `baseModel` 값**에서
  모은 것입니다(ltx·z-image·krea·wan·qwen·anima 를 그렇게 확인했습니다).
  새 엔진을 더할 때도 같은 방법으로: 그 엔진 이름으로 검색해 판의 baseModel 을 세어 보세요.
*/
/**
 * `hf` 는 허깅페이스 쪽 그물 — Civitai 에 없는 로라가 거기에 있습니다. `search` 는
 ...
 *
 * 늘 같은 로라로 돌리지 않습니다 — 컷마다 어울리는 것이 다르고, 한 번에 뽑기에서도
 * 상황에 맞게 갈아 끼울 수 있어야 합니다.
 *
 ...
 *
 * 시네마틱 풍과 애니메이션 풍은 섞이면 둘 다 흐려집니다.
 ...
 *
 * 화면에는 «프롬프트에 넣어야 먹습니다» 라고 적어 두고, 정작 보낼 때
